...
### Added
- **grok-headless adapter** — xAI Grok Build on subscription/OIDC auth (no `XAI_API_KEY` consumed): a 5th cheval headless voice routing flatline/FAGAN council/review inference through `grok` (grok-build default + grok-composer-2.5-fast) via `grok login`. New `xai` provider declares both models as `kind: cli`, chain-terminal (no cross-company fallback), unpriced/subscription-metered (#1057).
- **cursor-headless adapter** — Cursor Composer (Moonshot-Kimi base) in the cheval roster as a coding-specialist council voice on subscription auth with no API key consumed, mirroring the codex/gemini/claude-headless pattern; operators set `type: cursor-headless` directly (#966).
- **xAI added to the tier_groups router** as a selectable provider column (max/mid → grok-build, cheap/tiny → grok-fast) with deterministically zero change to default resolution — `_stage3_tier_groups` picks `sorted(mapping.keys())[0]` (anthropic always first) and `xai` sorts last (#1057).
- **claude-fable-5 registered in the dispatch SoT** as the top intelligence tier, pricing-first by design ($10/$50 per MTok) so it cannot meter as $0 and silently dominate the budget governor; includes a within-company fallback chain (→ opus-4-8 → sonnet-4-6 → headless) and the `fable` alias (#999).
- **cheval cost-attribution rollup CLI** — read-only reporting tool aggregating the cheval ledger's per-call agent+model+cost records into a table/JSON rollup (per agent/model/provider/day/trace) with a `--since` filter; every row carries `unpriced_calls` and table mode warns the cost column UNDERSTATES when non-zero (#1000).
- **Bedrock forward-routing operator documentation** (`docs/bedrock-routing.md`) — the one-flag recipe, the initial-vs-fallback gap that was fixed, the inverse-`fallback_to` mechanism, the misconfiguration guard, and a pre-merge interim workaround (#1055).
- **`cross_repo.allowed_owners` config option** for opt-in cross-org BridgeBuilder auto-fetches; the empty default preserves the secure org-only posture (#1014).
- **Injectable `CrossRepoExec` test seam** so the cross-repo allowlist/public/metadata gates can be proven without shelling out to real `gh` (#1014).
- **BridgeBuilder invocation trust contract** documented in the skill, recording why the #1050 "config from PR-head" finding was closed not-applicable (BridgeBuilder fetches PR data via `gh api` as diff-as-data and never checks out the PR head) so it is not re-raised (#1050).
- **Karpathy Simplicity First gains a 6-rung YAGNI escalation ladder** (need-it? → stdlib → native → installed-dep → one-line → minimum) plus an explicit "never simplify away" safety floor (trust-boundary validation, data-loss handling, security, accessibility) that cross-references the audit gate (#1067).
- **`loa:shortcut:` in-code marker convention** for deliberate simplifications, harvested by the existing `/ride` tech-debt scan, plus a runnable-check rule for non-trivial logic in Goal-Driven Execution and a 5-tag over-engineering review taxonomy (delete/stdlib/native/yagni/shrink); registered as machine-tracked constraint C-PROC-011 (#1067).
- **Stderr warning when `compliance_profile` is mis-nested** under `hounfour.*` instead of `providers.bedrock`, turning a silent no-op into an actionable message (#1055).
- **KF-016** (Bedrock-only silent quorum-degradation), **KF-017** (cross-model all-voice api_failure as auth/billing exhaustion, distinct from the KF-002 empty-content class, with a proven `LOA_HEADLESS_MODE=api-only` unblock), and a cross-reference to the prefer-api chain-walk resilience defect filed as #1071, all recorded in the known-failures log (#1055, #1072).
- **`resync <slug|--all>`** (alias `update`) command to force re-sync installed constructs from local source clones with a synced/skipped/failed report (#1021).
- **Construct-index generator regression coverage** (T13–T18) pinning event-key tolerance, construct.yaml overlay, `compose_with` seeding, scalar-event tolerance, and warn-on-malformed behavior (#981, #1012).
- **CI scanner for the BSD-incompatible mktemp class** (suffixed templates + GNU `-p`/`-t`) plus a bats suite, hardened so an `--exclude` arg cannot neuter the fence and a grep error fails loud (#978).
- **CLI-registry parity test for `_CLI_ADAPTER_BY_PROVIDER`** — the third headless registry the existing parity tests did not cover, so the next registry drift fails at lint instead of shipping config-dead (#1018).
- **Hook wiring-parity lint invariant (Invariant 7b)** failing CI on inert/unwired enforcement hooks, with `--root`/`--hooks-wiring-only` flags for read-only cross-tree wiring checks (#1002).
- **Research and proposal artifacts**: the honest-routing cycle sequencing plan (#1066), the 2026-06-12 15-agent/119-finding codebase refactor review with 14 ranked recommendations (no PR — direct commit), and the "Designing loops with Fable 5" research pointer mapping techniques to Loa primitives (#995).

### Changed
- **claude-headless now dispatches `fable` instead of `sonnet`** (`cli_model` sonnet → fable) per the 2026-06-11 operator decision, so every headless claude consumer (BridgeBuilder, the council/flatline claude voice, chain-walk fallbacks) runs the top tier on subscription; the separate `claude-fable-headless` entry and `fable-headless` alias from #1003 are removed, and all derived artifacts (sha256 lockfile, adapter maps, BB dist + manifest) are rebuilt (#1005).
- **Generalized cheval's CLI-adapter admission to a registry-derived `auth_type` contract** — the `kind:cli` escape hatch keys on `cli_adapter_types()` instead of the closed `_CLI_ADAPTER_BY_PROVIDER.values()` map, so the next headless adapter is admitted without editing closed dispatch values (#966).
- **Refreshed advisor/run_bridge/red_team Anthropic pins** claude-opus-4-7 → claude-opus-4-8 across the three `.loa.config.yaml` version-pins (#1058, #1070).
- **Qualified `status:clean` zero-findings adversarial verdicts** with an additive `status_note` ("no findings met the BLOCKING/ADVISORY bar — not an approval of unreviewed surface"), and `invoke_dissenter` now passes the real `--phase` to the model adapter instead of the cosmetic `prd` default (#1007).
- **Construct install prefers the local clone (`origin/main`) as source-of-truth**, with the registry demoted to last resort; local installs now succeed offline, and git sources install canonical `origin/main` content via `git archive` rather than working-tree state (#1021).
- **Refreshed the stale closed-#774 flatline doc-size warning text** to point at the streaming/chunking reality and `verdict_quality`/MODELINV diagnostics (#1007).
- **Construct-index `composes_with` is now seeded from explicit `compose_with[].slug` declarations**, with the reads/writes overlap unioned on top (previously always empty) (#981).
- **Encoded Three-Zone State-Zone write-freedom in the shipped permission allow-list** — 8 additive `Write`/`Edit` allow rules for grimoires/.beads/.run/.ck, so gate-skill artifact writes no longer fall through to session permission mode; System Zone stays protected, App Zone stays confirm-on-write (#1045).
- **Named [1.173.6] CHANGELOG entry + version-triangle re-sync** for the 1.173.1–1.173.6 tag-only patch rollup, surfacing previously-unchangelogged fixes (CLAUDE.loa.md token cut, next-bug-sprint-id collision, secret-scanning, Shell Tests deps) (#988, #942, #992, #886).

### Fixed
- **`compliance_profile` now governs INITIAL Bedrock routing, not just fallback** — under `prefer_bedrock`/`bedrock_only`, opus/sonnet aliases previously resolved to `anthropic:<model>` on the first dispatch; a Bedrock-only operator hit the dead Anthropic endpoint and a multi-voice Flatline/BridgeBuilder run silently scored a degraded auth-failure stub as a third voice. The loader now rewrites Anthropic-targeted aliases to their Bedrock equivalent at initial resolution via inverse `fallback_to` mapping, with the same transform mirrored in the bash codegen so flatline and the Python loader agree (#1055).
- **Granted reasoning-class (30-min) timeout to all `*-headless` BridgeBuilder aliases** — `isReasoningClass` missed the OAuth-subscription aliases (claude/codex/gemini-headless), so `deriveTimeoutMs` fell to the 300s cap and SIGTERM'd reviews on >100k-token diffs (KF-010 recurrence) (#1013).
- **adversarial-review finding-count failure now emits `status=malformed_response`** instead of clean-zero, routing parse failure / empty content / unparseable result to the fallback chain rather than aliasing it to SUCCESS; `merge_findings` refuses to merge against `[]` on a corrupt existing-findings file (#1028).
- **post-pr-triage emits a DEGRADED convergence record (never clean FLATLINE) on a corrupt findings artifact** — jq_strict migration at the finding-count, per-finding, and bridge-id sites forces `state=DEGRADED` with a parse-failure count and exit 3; a corrupt bridge-state.json overwrites any stale prior FLATLINE record before exiting (#1040).
- **red-team retention defaults unknown/mislabeled/missing classification to RESTRICTED** and aborts before any purge on missing dependencies, closing four bugs: indefinite retention of unparseable RESTRICTED files, over-permissive INTERNAL default, mass-purge on unavailable jq/compat-lib or bad `retention_days`, and forever-retention of future-timestamped files (#1040).
- **red-team code-vs-design gate writes a structured degraded record and exits non-zero** instead of silently passing degraded runs — validation now requires non-empty content AND an object with a findings array; model failure (including CHAIN_EXHAUSTED) writes a `{degraded:true, ...}` record and propagates through pipeline-self-review (#1007).
- **Emit a MODELINV audit envelope on the cheval preflight-preempt path** — the exit-7 preempt path returned before the try/finally that writes the envelope, so preemptions were invisible to `.run/model-invoke.jsonl`; a minimal fail-soft envelope is now emitted (#1053).
- **Hardened grok JSON envelope parsing** against log-line shadowing (parsing gates on a stable identifier: stopReason/sessionId/requestId) and non-string ids (numeric/null sessionId no longer leaks a non-string interaction_id) (#1057).
- **Routed cursor-headless prompt via stdin** to eliminate the ARG_MAX large-diff cliff (~256KB) that deterministically failed exactly the large-diff reviews this voice exists for, and bounded the 429 rate-limit probe so runs like "14290ms"/"429ms" don't misclassify a billed success (#966).
- **cheval headless subprocess salvage**: lone-surrogate input now raises immediately in the main thread (was swallowed on the daemon writer thread, burning the full timeout); killpg `PermissionError` falls back to `proc.kill()` without replacing the in-flight exception; all three `TimeoutExpired` raise sites now carry partial output/stderr (ported from PR #983) (#1011).
- **Kill the whole process tree on headless-adapter timeout** — `subprocess.run(timeout=)` killed only the immediate child, so agentic-CLI grandchildren survived as orphans holding the inherited pipes and blocked the post-kill drain for minutes; `run_subprocess_pgkill` (start_new_session, byte-capped select read, killpg SIGKILL) reaps the whole group (#982).
- **Repaired the uninstallable cheval adapter package** — `pip install .claude/adapters` died with BackendUnavailable because the build-backend pointed at a non-existent `setuptools.backends._legacy:_Backend`; corrected to `setuptools.build_meta` and declared the previously-empty runtime deps `rfc8785>=0.1.4` and `jsonschema>=4.21` (#982).
- **Cap-exceeded subprocess output now raises instead of returning truncated success** — exceeding `max_bytes` kills the process group and raises `SubprocessOutputCapExceeded`, converted to `ProviderUnavailableError` by all three adapters so the fallback chain advances like a timeout (#982).
- **Infer auth_type/dispatch_group/kind for custom `*-headless` providers** so operator-defined headless providers in `.loa.config.yaml` (including the framework's own documented example shape) no longer kill the entire multi-model lane at config load with `[CONFIG-INVALID]`; `[CONFIG-INVALID]` errors now also name the exact offending config path, and typed-headless providers dispatch by type on the `kind:cli` adapter path (#979).
- **Portable mktemp templates across the script tree** — BSD/macOS mktemp expands an X-run only when trailing, so suffixed templates created literal `-XXXXXX` files and died "File exists" (observed as flatline's 3-voice review silently degrading to voices=1/3); fixed via create-then-rename across invoke-diagnostics, flatline-orchestrator, lore-promote, semver-bump, audit-snapshot, red-team and others (#978).
- **Portable BUTTERFREEZONE freshness date parsing** — `butterfreezone-validate.sh` used GNU-only `date -d`, producing a false "~20618 days old" staleness WARN on every macOS run; the single freshness call site now routes through compat-lib `_date_to_epoch` (#1034).
- **Demote lifted reality headers so Tier-1 BUTTERFREEZONE docs pass the provenance census** — verbatim `## ` headers from reality files became untagged top-level sections, producing a deterministic "Missing provenance tags" FAIL; a portable heading-demotion sed now nests them under the tagged parent (#1035).
- **Resolved the construct-index `consumes` event membrane** (extractor used `.[].event` while manifests key on `.name`/`.type`) and **handled `.type`/`.name`/`.event` event shapes per-element** so emits/consumes are not null-poisoned, plus overlaying the construct.yaml event membrane onto manifest events (#981).
- **Replaced 9 stderr-swallowing extraction sites in construct-index-gen** with a warn-on-failure `_strict_or_default` helper, and tolerate scalar-string event arrays — malformed fields previously looked like legitimate empty values (KF-004/KF-015 output-swallow class) (#1012).
- **Make construct local-source materialization a true mirror** via a staging dir and atomic replace (prune deleted-upstream files, preserve `.license.json`), record the resolved `origin/main` commit in pack metadata, and clean up the meta tmp file on jq failure (#1021).
- **Match candidate local-source dirs against the requested slug in `find_local_source`** — any configured `local_source_paths` entry previously satisfied every slug, so post-#1021's atomic replace could mirror the WRONG pack's content over an installed pack (#1024).
- **Accept the full SemVer 2.0 grammar in release version detection and bumping** (arbitrary pre-release identifiers + `+build` metadata) and **restore SemVer §11 pre-release precedence** via `versionsort.suffix=-`, closing a silent post-merge tag/CHANGELOG/release skip (#745).
- **Golden-path and path-lib fail loud on unresolved grimoire dir / empty PROJECT_ROOT** instead of root-anchoring state paths at `/` and silently misdiagnosing phase as "discovery" (#980).
- **Reconcile gitignored `.claude` symlinks on submodule mounts in the post-merge CI pipeline** (a guarded reconcile step after each of the 3 checkouts, no `|| true`) and **non-destructively refresh the #842 copy set** on `--reconcile`/submodule update, with submodule-mode routing documented for `/update-loa` (#986, #968).
- **Preflight node_modules/zod in the BridgeBuilder entry point** on fresh mounts, replacing an opaque "Cannot find package zod" crash with a clear exit-3 `npm ci` remediation (#805).
- **Wire the @loa-managed kernel integrity-hash lifecycle** (stamp/verify/lint, consuming the `PLACEHOLDER` suffix and independently flagging residue) and **fix the pre-commit beads hook to flush from the main checkout in linked git worktrees**, flipping KF-014 OPEN → RESOLVED (#989, #991).
- **BB skip notices stamp a distinct skip marker** (`<marker>-skip: <sha>`) so a skip no longer permanently claims the sha and labeled re-runs / `--force-full-review` can reach an actual review (#1004).
- **SOUL.md (L7) validator accepts natural unquoted YAML dates** (an unquoted `last_updated: 2026-05-09` crashed every `soul_validate` via a `json.dumps` TypeError) and fails loud on ambiguous naive/fractional datetimes instead of silently stamping UTC (#1015).
- **Substrate-health 24h window filter reads `ts_utc`** (the writer's actual key) instead of the never-present `timestamp`/`ts`, so a "24h" health report is no longer silently all-time (#1018).
- **Synced the golden-path.sh eval fixture** after the #1017 fail-loud guards and **repaired v3-codegen byte-equality fixtures** missing required `auth_type`/`dispatch_group`, and **next-bug-sprint-id now scans the `sprint-bug-N/` dir layout** so the helper no longer re-emits an already-used sprint id (#1017, #1056, #1064).
- **Oversized CLI input always preempts (exit 7)**, turning the long-red preflight-gate P15/P16 tests green and mooting #1016 (#937).
- **Repointed the born-broken CLI-INSTALLATION.md link** in INSTALLATION.md to the public loa-constructs repo (#1020).

### Security
- **jq_strict fail-loud wrapper + CI tripwire** kill the `jq … 2>/dev/null || echo <default>` silent-clean pattern — the literal mechanism behind KF-004 (zero-findings canonical verdicts masking real findings, recurrence ≥20) and KF-015; `jq_strict` propagates jq's non-zero exit with an attributable diagnostic, and `check-no-swallowed-jq.sh` + a GitHub workflow block the pattern on the gate-critical script set (#1028).
- **Disabled the codex headless agent tool surface** (`shell_tool`/`browser_use`/`browser_use_external`/`computer_use`) to block prompt-driven file exfiltration via the agent's shell tool despite `--sandbox read-only` (empirically verified against codex 0.137.0: zero command_execution events, canary unreadable); **replaced the codex config-override passthrough with a default-deny allowlist + value-charset guard**; and **redact codex stderr before length-capping** with a tempdir-leak fix on spawn failure (#1008, #1049).
- **Run cursor-headless read-only and sandboxed** (`--mode plan`, `--sandbox enabled`, isolated empty cwd, own process group for SIGKILL, never `-f`/`--yolo`), with a transport-probe that excludes the model's own `result` field on success so a diff quoting "429"/"unauthorized" cannot silence the reviewer (#966).
- **Strip `XAI_API_KEY`, `GROK_CODE_XAI_API_KEY`, and `GROK_API_KEY` from the grok-headless subprocess env** to force the CLI onto its OIDC subscription path; test-pinned so a future base.py refactor can't silently drop a grok var (#1057).
- **Restrict BridgeBuilder auto-detected cross-repo fetches** parsed from the UNTRUSTED PR title — a confused-deputy that ran `gh issue view --repo <attacker-chosen>` with the bot credential — to an **org-only, fail-closed allowlist** (defaulting to the PR's own org), **public repos only** (private/internal/not-found/error collapse to one generic non-oracle message), and **metadata-only** (title/labels, no body); fetched content is **sanitized and wrapped in a neutralized `<<<UNTRUSTED CROSS-REPO DATA>>>` delimiter block** before reaching the review prompt. Manual operator-configured refs are unchanged (#1014).
- **Block inline agent writes to State-Zone executable/lifecycle paths across both permission surfaces** — the shipped Bash allow-list frictionlessly permitted direct writes to cron scripts (`.run/cron.d`), the sourced model-routing overlay (`.run/**/*.sh`), and the skill-approval lifecycle (`grimoires/loa/skills`); closed with 10 settings.json deny rules plus a block-destructive-bash FR-SZ guard (redirect/tee/copy-family/ln/dd/in-place-editors/archive-extraction), destination-anchored, with an audited `LOA_ALLOW_STATE_ZONE_EXEC_WRITE=1` escape hatch (#1046).
- **Wire zone-write-guard live** with the real stdin payload contract and exit-2 blocking — it was documented as running but wired nowhere, and even once wired resolved its target only from env/`$1` (Claude Code delivers the PreToolUse payload as JSON on stdin), so TARGET was always empty and every write was ALLOWED; it now reads `tool_input.file_path` from stdin, canonicalizes via `realpath -m` anchored to PROJECT_ROOT, and exits 2 (#1002).
- **Activate the adversarial-review-gate** (review-skip COMPLETED wall) in live settings — it was template-wired but absent from the shipped settings.json, so the wall had never fired; it now reads the FULL payload (a 64 KiB head-cap with `|| exit 0` previously fail-opened any COMPLETED write over 64 KiB), covers Edit/MultiEdit/NotebookEdit, and exits 2 to block (#1002).
- **post-pr-triage bridge_id charset guard before glob interpolation** plus control-char stripping at the triage log chokepoint (a newline-bearing bridge_id could forge a triage-log line), and **sanitize `run_id` before writing it to the red-team-retention audit log** (control-char strip + 512-byte cap) (#1053).
- **Extended `redact_secrets` coverage in invoke-diagnostics** — the AWS key-ID pattern now masks all 7 session/role prefix classes (AKIA|ASIA|AROA|AGPA|AIPA|ANPA|ANVA), not AKIA-only, and the Bearer char class includes `=` so base64-padded tokens redact fully instead of leaking padding past the match boundary (#1022).

### Removed
- **Deleted the dead `loa_cheval.chunking` package and the chunk-dispatch branch** — both doubly dead (gate unreachable, fail-closed by design) and falsely marked RESOLVED-STRUCTURAL via a source-grep test; the KF-002 oversized-input layer is covered by streaming transport + the 200K/180K walk gate. `_preflight_check` loses its `chunking_enabled` param and always preempts (exit 7); the `LOA_CHEVAL_DISABLE_CHUNKING` env var and `--force-chunking-placeholder` flag are removed. The MODELINV `chunk` enum and `chunked_review` field are retained-but-annotated-RETIRED so historical envelopes still validate (#937).
- **Removed the non-functional `cheval` console-script entry point** from `pyproject.toml` — it pointed at a non-existent module and the package was uninstallable beforehand, so no working command is removed; the real CLI is `.claude/adapters/cheval.py` invoked by path (#982).
