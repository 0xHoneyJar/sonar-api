...
export interface WorkerReturnContractValidation {
  result: 'PASS' | 'FAIL';
  errors: string[];
  canonicalValue: WorkerJsonValue | null;
}
...
function validateUnicode(value: unknown, path: string, errors: string[]): void {
  ...
  if (Array.isArray(value)) {
    value.forEach((entry, index) => (
      validateUnicode(entry, `${path}[${String(index)}]`, errors)
    ));
    return;
  }
...
class JsonMemberScanner {
  ...
  private scanObject(path: string): void {
    ...
    while (this.index < this.text.length) {
      ...
      if (seen.has(key)) this.duplicates.push(memberPath);
      seen.add(key);
      this.skipWhitespace();
      this.index += 1;
...
function nonemptyStringSchema(): WorkerJsonValue {
  return {
    type: 'string',
    pattern: '^[\\s\\S]*\\S[\\s\\S]*$',
  };
...
export function contractExemplarToJsonSchema(example: unknown): WorkerJsonValue {
  ...
  if (typeof example === 'string') {
    if (example === '') return nonemptyStringSchema();
...
function validateJudgmentRationale(
  value: unknown,
  path: string,
  errors: string[],
): void {
...
export function validateWorkerReturnContract(
  ...
  if (!isRecord(contractExemplar)) {
    ...
    try {
      ...
      errors.push(
        `Core output contract is malformed: ${
  ...
  try {
    if (text !== null) value = JSON.parse(text) as unknown;
  } catch (error) {
    errors.push(
      `worker return is invalid JSON: ${
        error instanceof Error ? error.message : String(error)
      }`,
    );
  }
  ...
  if (text !== null && value !== null && !errors.some((error) => /invalid JSON/u.test(error))) {
    try {
      ...
      errors.push(
        `worker return member scan failed: ${
          error instanceof Error ? error.message : String(error)
        }`,
      );
    }
  ...
  return {
    ...
    errors,
    canonicalValue: errors.length === 0 ? value as WorkerJsonValue : null,
  };
}
