...
function checkManifest(results: ResultCollector, model: RunModel): void {
  results.run('K2.2', 'manifest', (fail) => {
    ...
    const predecessorFields = manifest.lines.filter(
      (line) => /^\s*-\s*predecessor[_ -]run\s*:/i.test(line),
    );
    ...
    for (let i = 0; i < manifest.states.length; i++) {
      ...
      if (entered) previousTimestamp = entered;
      ...
      const repeatProjection = (
        (current === 'ACCEPTED' || current === 'PROJECTION-ACCEPTED')
        && state === 'PROJECTING'
      );
    ...
    if (projectionAcceptances.length > 0) {
      const p3Signoffs = manifest.signoffs.filter((row) => (
        positiveDecision(row.values.decision)
        && (/\bP3\b/i.test(row.values.gate) || /projection.*accept/i.test(row.values.gate))
      ));
      if (p3Signoffs.length < projectionAcceptances.length) {
        fail(
          `${projectionAcceptances.length} PROJECTION-ACCEPTED state(s) require at least `
  ...
  results.run('K2.4', 'packet resolution', (fail) => {
    ...
    for (const packet of model.packets) {
      ...
      if (scheme === 'md-lines') {
        ...
        if (!scheme) fail(`${sourceId} has no declared locator scheme`);
        else unverified.add(scheme);
      }
    }
...
function checkClaimShape(results: ResultCollector, model: RunModel): void {
  results.run('K2.6', 'claim table shape', (fail) => {
    ...
    for (const claim of model.claims) {
      const {
        claimId, packets, sources, claimType, disposition, status,
      } = claim.values;
...
function checkAccounting(results: ResultCollector, model: RunModel): void {
  results.run('K2.7', 'accounting', (fail) => {
    ...
    for (const row of model.dispositionRows) {
      ...
      if (rows.has(disposition)) fail(`duplicate disposition row "${disposition}" at ${location(row)}`);
...
function checkCriteria(results: ResultCollector, model: RunModel): void {
  results.run('K2.9', 'criteria precede packets', (fail) => {
    ...
    if (!writtenTimestamp) fail(`written timestamp "${written || '(blank)'}" is invalid`);
    ...
    if (supersessions && /supersession/i.test(model.criteria.text)) {
      for (const row of supersessions.rows) {
        if (row.cells.length < 4 || !row.cells[0].trim()) continue;
  ...
  results.run('K2.10', 'status discipline', (fail) => {
    ...
    for (const [family, byId] of homeRows) {
      for (const row of byId.values()) {
        if (!row.status.startsWith('superseded-by:')) continue;
...
function checkPrecis(results: ResultCollector, model: RunModel): void {
  results.run('K2.11', 'precis consistency', (fail) => {
    ...
    if (missing.length) fail(`missing envelope section(s) ${missing.join(', ')}`);
    ...
    for (let index = 0; index < model.precis.lines.length; index++) {
      ...
      if (
        PROJECTION_TERMS.some((pattern) => pattern.test(line))
    ...
    for (const entry of precisRows) {
      ...
      if (seen.has(entry.id)) fail(`§4 defines ${entry.id} more than once`);
      seen.add(entry.id);
    ...
    for (const claim of active) {
      if (!seen.has(claim.values.claimId)) fail(`§4 is missing active claim ${claim.values.claimId}`);
    }

    const ids = new Set(precisRows.map((entry) => entry.id));
    for (const id of idsIn(text, 'CC')) {
      if (!ids.has(id)) fail(`C1 phantom CC: ${id} is referenced but not defined in §4`);
    }
    let outside = text;
    for (const number of [4, 5]) {
      const section = envelopeSection(text, number);
      if (section) outside = outside.replace(section, '');
    }
    for (const id of ids) {
    ...
    const ledgerTable = findTableByFirstHeader(
      parseTables(envelopeSection(text, 5), 'precis.md'),
      'disposition',
    );
    ...
    if (!ledgerTable) {
      ...
      for (const row of ledgerTable.rows) {
        ...
        if (!Number.isInteger(count) || count !== actualCounts.get(disposition)) {
          fail(
            `C3 ledger count: §5 declares ${row.cells[1] || '(blank)'} ${disposition}, `
      ...
      if (!Number.isInteger(declaredTotal) || declaredTotal !== precisRows.length) {
        fail(
          `C3 ledger total: §5 declares ${declaredTotal ?? '(missing)'}, `
          + `recomputed ${precisRows.length}`,
        );
      }
      for (const id of ids) {
        if (!ledgerIds.has(id)) fail(`C3 ledger coverage: ${id} is absent from §5`);
      }
    }
    ...
    if (!sourceTable) {
      ...
      for (const row of sourceTable.rows) {
        if (/^SRC-\d+$/.test(row.cells[0] || '')) precisSourceIds.add(row.cells[0]);
      }
    }
    ...
    for (const id of idsIn(text, 'SRC')) {
      if (!precisSourceIds.has(id)) fail(`C4 phantom SRC: ${id} does not resolve to Précis §2`);
    }
    ...
    if (matrixTable) {
      ...
      for (const row of matrixTable.rows) {
        ...
        if (/^STM-\d+$/.test(stm)) stmRows.add(stm);
        if (ccIndex >= 0) {
          for (const id of idsIn(row.cells[ccIndex] || '', 'CC')) {
            if (!ids.has(id)) fail(`C5 matrix CC ref: ${stm} references missing ${id}`);
          }
        }
        if (srcIndex >= 0) {
          for (const id of idsIn(row.cells[srcIndex] || '', 'SRC')) {
            if (!precisSourceIds.has(id)) fail(`C6 matrix SRC ref: ${stm} references missing ${id}`);
          }
        }
      }
    }
    for (const id of idsIn(text, 'STM')) {
      if (!stmRows.has(id)) fail(`C7 phantom STM: ${id} is not a matrix row`);
    }
    ...
    const mergeTable = findTableByFirstHeader(
      parseTables(envelopeSection(text, 11), 'precis.md'),
      'canonical',
    );
    if (mergeTable) {
