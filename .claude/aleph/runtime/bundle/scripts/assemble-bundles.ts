...
const HOST_ADAPTER_IDS = ['loa', 'hermes'] as const;
...
export interface BundleSetVerificationReport {
  result: 'PASS' | 'FAIL';
  bundles: BundleVerificationReport[];
  errors: string[];
}
...
function parseCli(args: string[]): CliOptions {
  ...
  if (command === 'assemble' || command === 'verify') options.command = command;
  ...
  for (let index = 0; index < rest.length; index += 1) {
    ...
    if (arg === '--json') options.json = true;
      ...
      const value = rest[index + 1];
      if (!value) options.error = '--output requires a directory';
      else {
      ...
      const value = rest[index + 1];
      if (!value) options.error = '--bundle requires a directory';
      else {
...
function failureMessages(report: ReturnType<typeof validateCoreBoundary>): string[] {
...
function loadSource(
  ...
  plans: BundlePlan[];
  report: ReturnType<typeof validateCoreBoundary>;
} {
  const report = validateCoreBoundary({ root });
  if (report.result !== 'PASS') {
    throw new Error(`source Core-boundary validation failed: ${
      failureMessages(report).join('; ')
    }`);
  }
  const manifest = readJsonFile(join(root, 'core.manifest.json')) as CoreManifest;
  const plans = manifest.bundle_targets
    .map((target) => {
      ...
      const adapter = readJsonFile(adapterPath) as AdapterManifest;
      return buildBundlePlan(root, manifest, target, adapter);
    })
    .sort((left, right) => utf8Compare(left.target.id, right.target.id));
  if (plans.length !== 2
    || !sameStrings(
      plans.map((plan) => plan.target.id),
      ['aleph-for-loa', 'aleph-for-hermes'],
    )) {
  ...
  const coreInventories = new Set(plans.map((plan) => canonicalJson(
    plan.files.filter((file) => file.classification === 'core'),
  )));
...
function assertIgnoredOutput(root: string, output: string): void {
  if (!pathInside(root, output)) return;
  const relativeOutput = relative(root, output);
  if (!relativeOutput) throw new Error('bundle output may not replace the source root');
  ...
  if (result.status !== 0) {
    throw new Error(
      `bundle output inside the repository must be ignored: ${relativeOutput}`,
    );
  }
}

function sourceSnapshot(plans: BundlePlan[], root: string): Map<string, Buffer> {
  ...
  for (const [path, record] of [...records].sort(([left], [right]) => (
...
function writeStagedBundle(
  ...
  for (const file of plan.files) {
    ...
    if (!bytes) throw new Error(`source snapshot omitted ${file.path}`);
...
function recursiveFiles(root: string): { files: string[]; errors: string[] } {
  const files: string[] = [];
  const errors: string[] = [];
...
function validateDigest(value: unknown, scope: string, errors: string[]): void {
  if (typeof value !== 'string' || !SHA256_PATTERN.test(value)) {
    errors.push(`${scope} must be sha256:<64 lowercase hex>`);
  }
}

function validateLockStructure(value: unknown, errors: string[]): BundleLock | null {
  if (!exactKeys(value, [
    'lock_format',
    'digest_algorithm',
    'lock_digest',
  ...
  const lock = value as unknown as BundleLock;
  ...
  if (!exactKeys(lock.bundle, [
    ...
    if (typeof lock.bundle.id !== 'string' || !/^aleph-for-[a-z][a-z0-9-]*$/.test(
      lock.bundle.id,
    )) errors.push('bundle.id is malformed');
    if (typeof lock.bundle.version !== 'string') errors.push('bundle.version is malformed');
  ...
  if (!exactKeys(lock.core, ['id', 'version', 'tree_digest'])) {
    ...
    if (typeof lock.core.id !== 'string') errors.push('core.id is malformed');
    if (typeof lock.core.version !== 'string') errors.push('core.version is malformed');
  ...
  if (!exactKeys(lock.adapter, ['id', 'version', 'lifecycle', 'tree_digest'])) {
    ...
    if (!['planned', 'implemented', 'validated', 'sanctioned'].includes(
      lock.adapter.lifecycle,
    )) {
  ...
  if (!exactKeys(lock.source, [
    ...
    validateDigest(
      ...
      'source.manifest_projection_digest',
      errors,
    );
  ...
  if (!exactKeys(lock.provenance, ['format', 'vcs', 'digest'])) {
    ...
    if (!exactKeys(vcs, [
      ...
      if (vcs.kind !== 'git-dependency-closure-snapshot') {
        errors.push(
          'provenance.vcs.kind must be git-dependency-closure-snapshot',
        );
      }
  ...
  if (!Array.isArray(lock.files) || lock.files.length === 0) {
    ...
    if (new Set(paths).size !== paths.length) errors.push('files paths must be unique');
...
function adapterForeignProblems(
  bundleRoot: string,
  lock: BundleLock,
): string[] {
...
function decodedAdapterForeignProblems(
  ...
  for (const foreignId of HOST_ADAPTER_IDS.filter((id) => id !== adapterId)) {
    ...
    if (values.some((value) => (
      value.includes(`adapters/${foreignId}/`)
...
function emittedBoundaryReport(
  bundleRoot: string,
  lock: BundleLock,
  ...
  try {
    ...
    writeFileSync(
      join(tempRoot, 'core.manifest.json'),
      `${JSON.stringify(lock.source.manifest_projection, null, 2)}\n`,
    );
    ...
    if (report.result !== 'PASS') {
      throw new Error(
        `emitted Core-boundary validator failed: ${JSON.stringify(report.checks)}`,
      );
    }
...
function compareBoundaryDigests(
...
export function verifyBundle(bundlePath: string): BundleVerificationReport {
  ...
  let value: unknown;
  try {
    value = JSON.parse(rawLock.toString('utf8')) as unknown;
  } catch (error) {
    return {
      result: 'FAIL',
      bundlePath: bundleRoot,
      errors: [`bundle.lock.json is invalid JSON: ${
  ...
  try {
    ...
    return {
      result: 'FAIL',
      bundlePath: bundleRoot,
      errors: [`bundle.lock.json cannot be canonically serialized: ${
        error instanceof Error ? error.message : String(error)
      }`],
    };
  }
  ...
  const actualFiles: BundleFileRecord[] = [];
  for (const file of lock.files) {
    const absolute = join(bundleRoot, file.path);
    ...
    if (digest !== file.digest) errors.push(`modified bundle file: ${file.path}`);
  ...
  const projectionAdapterPaths = projectionAdapters
    ...
    ? projectionAdapters[lock.adapter.id] as string[]
    : [];
  ...
  const target = projectionTargets[0] as CoreManifest['bundle_targets'][number] | undefined;
  ...
  const coreDigest = digestEntries(
    actualFiles.filter((file) => file.classification === 'core'),
  );
  const adapterDigest = digestEntries(
    actualFiles.filter((file) => file.classification === 'adapter'),
  );
  const payloadDigest = digestEntries(actualFiles);
  const checkerPathSet = new Set(projectionCheckerPaths);
  const checkerDigest = digestEntries(
    actualFiles.filter((file) => checkerPathSet.has(file.path)),
  );
  if (coreDigest !== lock.core.tree_digest) errors.push('Core tree digest mismatch');
  ...
  if (checkerDigest !== lock.checker_digest) errors.push('checker digest mismatch');
  if (lock.source.manifest_projection_digest !== sha256Digest(
    Buffer.from(`${canonicalJson(projection)}\n`, 'utf8'),
  )) {
    errors.push('source manifest projection digest mismatch');
  }
  const toolRecord = actualFiles.find(
  ...
  if (resealed.lock_digest !== lock.lock_digest) errors.push('lock digest mismatch');
  if (resealed.bundle.digest !== lock.bundle.digest) errors.push('bundle digest mismatch');
  ...
  if (!adapterPaths.includes(adapterManifestPath)) {
    ...
    try {
      const adapter = readJsonFile(
        join(bundleRoot, adapterManifestPath),
      ) as AdapterManifest;
      ...
      errors.push(`emitted adapter manifest is invalid JSON: ${
        error instanceof Error ? error.message : String(error)
      }`);
    }
  }
...
function compareEmittedCore(
  bundleRoots: string[],
  reports: BundleVerificationReport[],
): void {
  if (bundleRoots.length !== 2 || reports.length !== 2) {
    throw new Error('release comparison requires both host bundles');
  }
  const locks = bundleRoots.map((root) => (
    readJsonFile(join(root, 'bundle.lock.json')) as BundleLock
  ));
  const coreInventories = locks.map((lock) => (
...
export function verifyBundleSet(
  ...
  if (bundlePaths.length === 2 && reports.every((report) => (
    ...
    if (!sameStrings(
      ...
      errors.push(
        'release verification requires exactly aleph-for-loa and aleph-for-hermes',
      );
    } else {
      try {
        compareEmittedCore(
          pairs.map((pair) => pair.path),
          pairs.map((pair) => pair.report),
        );
      } catch (error) {
        errors.push(error instanceof Error ? error.message : String(error));
      }
...
export function assembleBundles(
  ...
  outputRoot = join(sourceRoot, '.aleph-bundles'),
): AssemblyReport {
  ...
  try {
    ...
    for (const report of verification) {
      if (report.result !== 'PASS' || !report.summary) {
        throw new Error(
          `staged verification failed for ${report.bundlePath}: ${
            report.errors.join('; ')
          }`,
        );
      }
    ...
    for (const [index, plan] of source.plans.entries()) {
      ...
      if (final.result !== 'PASS' || !final.summary) {
        throw new Error(
          `final verification failed for ${destination}: ${final.errors.join('; ')}`,
        );
      }
...
function printBundleSummary(prefix: string, summary: BundleSummary): void {
  console.log(`${prefix} ${summary.id} ${summary.path}`);
  console.log(`DIGEST core ${summary.coreDigest}`);
  console.log(`DIGEST adapter:${summary.id.replace('aleph-for-', '')} ${
    summary.adapterDigest
  }`);
  ...
  console.log(
    `PREFLIGHT ${summary.id.replace('aleph-for-', '')} ${summary.preflight} `
...
function main(): void {
  ...
  if (options.command === 'assemble') {
    ...
    if (options.json) console.log(JSON.stringify(report, null, 2));
      ...
      for (const summary of report.bundles) printBundleSummary('ASSEMBLED', summary);
