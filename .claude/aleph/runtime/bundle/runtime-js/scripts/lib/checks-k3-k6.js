...
import { activeClaims, fieldValue, location, makeIndexes, normalizeSha256, pathIsWithin, parseTimestamp, compareTimestamp, resolveRenderedPath, sha256, } from './check-helpers.js';
import { envelopeSection, findTableByFirstHeader, idsIn, normalizeHeader, parseBulletFields, parseFieldTable, parseTables, renderedParagraphs, tableCells, isSeparatorRow, } from './markdown.js';
const ROLES = [
    'load-bearing',
    'corroborative',
    'contradictory',
...
function evidenceState(model) {
    ...
    for (const edge of activeEdges) {
        if (!edgesByClaim.has(edge.values.claimId))
            edgesByClaim.set(edge.values.claimId, []);
    ...
    const targetClaims = [...active.values()].filter((claim) => (claim.values.disposition === 'carried' || claim.values.disposition === 'merged'));
...
function runK3Checks(results, model) {
    results.run('K3.1', 'edge shape', (fail) => {
        ...
        if (table.header.length !== 7)
            fail(`edge header has ${table.header.length} columns, expected 7`);
        for (const edge of model.evidence.edges) {
            ...
            if (superseded) {
                const family = superseded[1].startsWith('CC-') ? 'CC' : 'SRC';
                const target = indexes[family].get(superseded[1]);
                const targetIsActive = (family === 'SRC'
    ...
    results.run('K3.4', 'coverage', (fail) => {
        ...
        for (const key of Object.keys(actual)) {
            if (declared[key] === null)
                fail(`coverage accounting is missing "${key}"`);
        ...
        for (const merge of model.merges.filter((row) => row.values.status === 'active')) {
            ...
            for (const absorbed of idsIn(merge.values.absorbs, 'CC')) {
                ...
                for (const edge of contradictory) {
                    if (!canonicalEdges.some((candidate) => (candidate.values.sourceId === edge.values.sourceId
    ...
    results.run('K3.8', 'inference markers resolve', (fail) => {
        ...
        for (const marker of model.evidence.markers) {
            ...
            if (basis.length === 0)
                fail(`${marker.values.claimId} marker has no CC/PKT basis`);
            for (const id of basis) {
                const family = id.startsWith('CC-') ? 'CC' : 'PKT';
                if (!indexes[family].has(id))
                    fail(`${marker.values.claimId} marker basis ${id} does not resolve`);
            }
...
function cardReferentIds(model, cards) {
    ...
    for (const [id, data] of cards) {
        byCard.set(id, new Set(data.refs));
    }
...
function runK4Checks(results, model) {
            ...
            if (!card.fieldTable.table) {
                ...
                for (const field of expectedFields) {
                    if (!fieldNames.includes(field))
                        fail(`${card.id} is missing field "${field}"`);
                }
        ...
        for (const data of cards.values()) {
            ...
            if (data.posture === 'unrouted-pending-external-referent') {
                if (relatedRefs.length === 0)
                    fail(`${data.card.id} pending posture lists no REF`);
            ...
            for (const id of relatedRefs) {
                ...
                if (['supplied', 'declined'].includes(ref.values.status)) {
                    ...
                    for (const match of data.history.matchAll(pattern))
    ...
    results.run('K4.4', 'tags are tags', (fail) => {
        const table = model.tagDocument
            ...
            : null;
        if (!table)
            fail('pre-cluster-tags.md has no tag table');
        else {
            if (table.header.length !== 3)
                fail(`tag header has ${table.header.length} columns, expected 3`);
            for (const row of table.rows) {
                if (row.cells.length !== 3)
                    fail(`tag row has ${row.cells.length} columns at ${location(row)}`);
            }
        }
        for (const file of model.files) {
            if (file.relativePath.startsWith('clusters/')
    ...
    results.run('K4.5', 'dependency sanity', (fail) => {
        ...
        for (const data of cards.values()) {
            ...
            for (const dependency of data.deps) {
                if (!indexes.RC.has(dependency))
                    fail(`${data.card.id} depends on missing ${dependency}`);
    ...
    results.run('K4.6', 'posture history', (fail) => {
        for (const data of cards.values()) {
            ...
            if (occurrences.length === 0)
                fail(`${data.card.id} has no posture-history entry`);
...
function routingState(model) {
    ...
    for (const [id, data] of cards.entries()) {
        ownUnresolved.set(id, new Set([...(cardRefs.get(id) || [])]
    ...
    const coneRefs = (id, visiting = new Set()) => {
        if (memo.has(id))
            return new Set(memo.get(id));
        if (visiting.has(id))
            return new Set(ownUnresolved.get(id) || []);
        visiting.add(id);
        const result = new Set(ownUnresolved.get(id) || []);
        for (const dependency of cards.get(id)?.deps || []) {
            for (const ref of coneRefs(dependency, visiting))
                result.add(ref);
        }
        visiting.delete(id);
        memo.set(id, result);
    ...
    const coneCards = (id, visiting = new Set()) => {
        if (cardMemo.has(id))
            return new Set(cardMemo.get(id));
        if (visiting.has(id))
            return new Set([id]);
        visiting.add(id);
        const result = new Set([id]);
        for (const dependency of cards.get(id)?.deps || []) {
            for (const cardId of coneCards(dependency, visiting))
                result.add(cardId);
        }
        visiting.delete(id);
...
function artifactCone(file, routing) {
    ...
    for (const [id, data] of routing.cards.entries()) {
        if (data.claims.some((claim) => claimIds.has(claim))
    ...
    const visit = (id) => {
        if (all.has(id))
            return;
        all.add(id);
        for (const dependency of routing.cards.get(id)?.deps || [])
            visit(dependency);
    };
...
function runK5Checks(results, model) {
    ...
    results.run('K5.2', 'gate', (fail) => {
        const surfaces = model.files.filter((file) => (file.relativePath === 'precis.md'
        ...
        for (const file of surfaces) {
            if (!/externally complete/i.test(file.text))
                continue;
    ...
    results.run('K5.3', 'Precis taint honesty', (fail) => {
        if (!model.precis)
            return 'precis.md not present; taint rendering not yet applicable';
        const active = new Map(activeClaims(model).map((claim) => [claim.values.claimId, claim]));
        ...
        for (const ref of [...relevantRefs, ...declined]) {
            if (!new RegExp(`\\b${ref}\\b`).test(section))
                fail(`precis.md §17 does not name ${ref}`);
        }
    ...
    results.run('K5.4', 'resolution hygiene', (fail) => {
        ...
        for (const referent of model.referents) {
            if (referent.values.status !== 'supplied')
                continue;
            ...
            const signoffBacked = Boolean(intake && model.manifest?.signoffs.some((row) => {
                ...
                const cited = ((row.values.gate.trim() && intake.includes(row.values.gate.trim()))
...
function runK6Checks(results, model) {
    ...
    results.run('K6.1', 'commission', (fail) => {
        ...
        for (const projection of byType) {
            ...
            if (projectionIdRows.length !== 1) {
                fail(`${projection.type} commission has ${projectionIdRows.length} projection id rows, `
            ...
            if (!/^PRJ-\d+$/.test(projectionId)) {
                ...
                fail(`${projection.type} commission repeats ${projectionId} from `
            ...
            if (fields.get('projection type') !== projection.type) {
                fail(`${projection.type} commission declares projection type `
            ...
            if (fields.get('projection trace') !== expectedTrace) {
                fail(`${projection.type} commission projection trace `
            ...
            if (traceProjectionId !== projectionId) {
                fail(`${projection.type} trace projection id `
                    + `"${traceProjectionId || '(blank)'}" does not match ${projectionId || '(blank commission id)'}`);
            }
            const declaredHash = normalizeSha256(fields.get('precis hash at commissioning')
            ...
            if (model.manifest) {
                ...
                if (!accepted)
                    fail(`${projection.type} commission has no preceding ACCEPTED state`);
                else if (!date)
                    fail(`${projection.type} commission date is invalid`);
    ...
    results.run('K6.2', 'selection coverage', (fail) => {
        const carried = [...active.values()].filter((claim) => (claim.values.disposition === 'carried' || claim.values.disposition === 'merged'));
        for (const projection of byType) {
            ...
            for (const row of projection.selection.rows) {
                ...
                if (!claim) {
                    ...
                    fail(`${projection.type} ${row.values.claimId} declares disposition `
                ...
                if (row.values.selection === 'not-used'
            ...
            for (const claim of carried) {
                ...
                if (!row)
                    fail(`${projection.type} selection is missing ${claim.values.claimId}`);
        ...
        for (const projection of byType) {
            ...
            for (const row of projection.trace.rows) {
                ...
                if (row.values.kind === 'load-bearing') {
                    ...
                    if (backing.length === 0)
                        fail(`${projection.type} open-item ${row.values.anchor} has no backing`);
                    for (const id of backing) {
                        const claim = active.get(id);
                        if (!claim || !['deferred', 'unresolved'].includes(claim.values.disposition)) {
        ...
        for (const projection of byType) {
            ...
            for (const anchor of paragraphs) {
                if (!anchors.has(anchor))
                    fail(`${projection.type} rendered paragraph ${anchor} has no trace row`);
            }
            for (const anchor of anchors) {
                if (!paragraphs.has(anchor))
                    fail(`${projection.type} trace anchor ${anchor} does not exist`);
            }
        }
    ...
    results.run('K6.6', 'boundary honor', (fail) => {
        ...
        for (const boundary of model.boundaries) {
            if (boundary.values.type === 'do-not-use-harm' && boundary.values.status === 'active') {
                for (const id of idsIn(boundary.values.governs, 'CC'))
                    prohibited.add(id);
            }
        }
        for (const projection of byType) {
    ...
    results.run('K6.7', 'open voice', (fail) => {
        for (const projection of byType) {
            ...
            const openRows = new Map([...selections].filter(([, row]) => row.values.selection === 'surfaced-as-open'));
            ...
            for (const [id, selection] of openRows) {
                ...
                const traces = openTraces.filter((row) => (row.values.kind === 'open-item' && idsIn(row.values.backing, 'CC').includes(id)));
                if (traces.length === 0)
                    fail(`${projection.type} ${id} has no open-item trace row`);
            }
        }
    ...
    results.run('K6.8', 'taint propagation', (fail) => {
        for (const projection of byType) {
            const used = new Set((projection.selection?.rows || [])
                .filter((row) => row.values.selection === 'used')
                .map((row) => row.values.claimId));
            const tainted = [...routing.cards.entries()].some(([id, card]) => (card.claims.some((claim) => used.has(claim)) && routing.coneRefs(id).size > 0));
            if (!tainted)
                continue;
            const path = traceDocumentPath(model, projection);
            if (!path || !existsSync(path))
    ...
    results.run('K6.9', 'neutrality both ways', (fail) => {
        for (const projection of byType) {
            ...
            if (!path || !existsSync(path))
                continue;
            const text = readFileSync(path, 'utf8');
            if (/\bPKT-\d+\b/.test(text))
                fail(`${projection.type} rendered document leaks packet IDs`);
            for (const [index, line] of text.split('\n').entries()) {
                const cells = tableCells(line);
                if (!cells || isSeparatorRow(cells) || cells.length !== 4)
                    continue;
                if (cells.some((cell) => [
    ...
    results.run('K6.10', 'type contract', (fail) => {
        for (const projection of byType) {
            ...
            if (fields.get('projection type') !== projection.type) {
                fail(`${projection.type} rendered metadata declares type `
                    + `"${fields.get('projection type') || '(blank)'}"`);
            }
            ...
            if (sections.join(',') !== contract.sections.join(',')) {
                fail(`${projection.type} top-level sections are [${sections.join(', ')}], `
