...
const ENTRYPOINT_KINDS = ['slash-command', 'native-agent', 'executable', 'tool'];
...
function validateIdentifier(value, scope, errors) {
    if (typeof value !== 'string' || !IDENTIFIER_PATTERN.test(value)) {
        errors.push(`${scope} must match ${IDENTIFIER_PATTERN.source}`);
    }
}
function validateVersion(value, scope, errors) {
    if (typeof value !== 'string' || !VERSION_PATTERN.test(value)) {
        errors.push(`${scope} must be a semantic version`);
    }
}
function validateRepositoryPath(value, scope, errors, allowNull = false) {
    if (allowNull && value === null)
        return;
    if (typeof value !== 'string' || !normalizedRepositoryPath(value)) {
        errors.push(`${scope} must be a normalized repository-relative path${allowNull ? ' or null' : ''}`);
    }
}
function validateRepositoryPathList(value, scope, errors, requireNonempty = false) {
    ...
    if (new Set(value).size !== value.length)
        errors.push(`${scope} must contain unique paths`);
...
function rank(value) {
    return LIFECYCLES.indexOf(value);
}
...
function validateCoreManifestShape(value) {
    const errors = [];
    if (!exactKeys(value, [
        'manifest_format',
        'core',
    ...
    if (!exactKeys(value.core, [
        ...
        validateVersion(value.core.adapter_protocol_version, 'core.adapter_protocol_version', errors);
    ...
    if (!exactKeys(value.manual_execution_binding, [
        ...
        validateIdentifier(value.manual_execution_binding.id, 'manual_execution_binding.id', errors);
        validateVersion(value.manual_execution_binding.version, 'manual_execution_binding.version', errors);
        if (value.manual_execution_binding.lifecycle !== 'sanctioned') {
            errors.push('manual_execution_binding.lifecycle must be sanctioned');
        }
        validateRepositoryPathList(value.manual_execution_binding.paths, 'manual_execution_binding.paths', errors, true);
    }
    if (!exactKeys(value.files, [
        ...
        validateRepositoryPathList(value.files.repository_administration, 'files.repository_administration', errors);
        if (!isRecord(value.files.adapter)) {
            ...
            for (const [adapterId, paths] of Object.entries(value.files.adapter)) {
                ...
                validateRepositoryPathList(paths, `files.adapter.${adapterId || '(blank)'}`, errors, true);
            }
        }
    }
    ...
    if (!Array.isArray(value.bundle_targets) || value.bundle_targets.length === 0) {
        ...
        for (const [index, target] of value.bundle_targets.entries()) {
            if (!exactKeys(target, ['id', 'version', 'adapter_id'])) {
                ...
                validateVersion(target.version, `bundle_targets[${index}].version`, errors);
                validateIdentifier(target.adapter_id, `bundle_targets[${index}].adapter_id`, errors);
            }
        }
    }
    return errors;
}
function asCoreManifest(value) {
    return validateCoreManifestShape(value).length === 0
        ? value
        : null;
}
function adapterShapeErrors(value) {
    const errors = [];
    if (!exactKeys(value, [
        '$schema',
        'manifest_format',
    ...
    if (!exactKeys(value.core_consumption, [
        ...
        validateRepositoryPathList(value.core_consumption.overrides, 'core_consumption.overrides', errors);
        validateRepositoryPathList(value.core_consumption.duplicates, 'core_consumption.duplicates', errors);
        validateRepositoryPathList(value.core_consumption.transformations, 'core_consumption.transformations', errors);
    }
        ...
        for (const [index, entrypoint] of value.entrypoints.entries()) {
            if (!exactKeys(entrypoint, ['id', 'kind', 'state', 'path'])) {
                ...
                if (typeof entrypoint.kind !== 'string'
                    || !ENTRYPOINT_KINDS.includes(entrypoint.kind)) {
                    errors.push(`entrypoints[${index}].kind must be one of ${ENTRYPOINT_KINDS.join(', ')}`);
                }
                if (typeof entrypoint.state !== 'string' || !lifecycle(entrypoint.state)) {
                    errors.push(`entrypoints[${index}].state is not a valid lifecycle`);
                }
                validateRepositoryPath(entrypoint.path, `entrypoints[${index}].path`, errors, true);
            }
        }
    }
    ...
    if (!isRecord(value.capabilities)) {
        ...
        for (const capability of CAPABILITIES) {
            ...
            if (!exactKeys(record, ['state', 'evidence'])) {
                ...
                validateRepositoryPathList(record.evidence, `capabilities.${capability}.evidence`, errors);
            }
        }
    }
    ...
    if (!Array.isArray(value.profiles)) {
        ...
        for (const [index, profile] of value.profiles.entries()) {
            ...
            validateRepositoryPathList(profile.evidence, `profiles[${index}].evidence`, errors);
        }
    }
...
function asAdapterManifest(value) {
    return adapterShapeErrors(value).length === 0
        ? value
        : null;
}
function pathOwner(ownership, path) {
    return ownership.get(path);
}
function referenceAllowed(ownership, path, adapterId) {
...
function allAdapterReferences(manifest) {
    const paths = [
        ...manifest.references,
    ...
    if (manifest.installation.path)
        paths.push(manifest.installation.path);
    for (const capability of Object.values(manifest.capabilities)) {
        paths.push(...capability.evidence);
    }
    for (const profile of manifest.profiles) {
        if (isRecord(profile) && typeof profile.path === 'string')
            paths.push(profile.path);
        if (isRecord(profile) && isStringArray(profile.evidence))
            paths.push(...profile.evidence);
    }
    return sortedUnique(paths);
}
function lifecycleProblems(root, adapterId, manifest, ownership) {
    const problems = [];
...
export function validateCoreBoundary(options = {}) {
    ...
    results.run('CB1', 'core manifest', (fail) => {
        ...
        try {
            ...
            fail(`core.manifest.json is not valid JSON: ${error instanceof Error ? error.message : String(error)}`);
            return 'core manifest checked';
        }
        for (const error of validateCoreManifestShape(coreValue))
            fail(error);
    ...
    extra.inventory = {
        ...
        adapters: Object.fromEntries(Object.entries(manifest.files.adapter).map(([id, paths]) => [id, paths.length])),
    ...
    results.run('CB2', 'inventory classification', (fail) => {
        ...
        for (const path of classifiedPaths) {
            if (!normalizedRepositoryPath(path))
                fail(`classified path is not normalized: ${path}`);
        }
        for (const path of duplicatePaths)
            fail(`path has multiple classifications: ${path}`);
        const actual = new Set(inventory.paths);
        for (const path of inventory.paths) {
            if (!ownership.has(path))
                fail(`unclassified tracked/nonignored path: ${path}`);
            if (!normalizedRepositoryPath(path))
                fail(`inventory path is not normalized: ${path}`);
            const absolute = join(root, path);
            if (!existsSync(absolute))
                fail(`inventoried path is missing: ${path}`);
            else if (lstatSync(absolute).isSymbolicLink())
                fail(`symlink is not allowed: ${path}`);
            else if (!lstatSync(absolute).isFile())
                fail(`inventory entry is not a file: ${path}`);
        }
        for (const path of classifiedPaths) {
            if (!actual.has(path))
                fail(`classified path is absent from git inventory: ${path}`);
        }
        ...
        for (const path of inventory.paths) {
            ...
            if (path.startsWith('docs/')
                ...
                if (owner?.classification !== 'core')
                    fail(`Core-owned path is not Core: ${path}`);
            }
    ...
    results.run('CB4', 'checker and typed manifest references', (fail) => {
        for (const path of manifest.checker_paths) {
            if (!normalizedRepositoryPath(path))
                fail(`checker path is not normalized: ${path}`);
            if (pathOwner(ownership, path)?.classification !== 'core') {
                fail(`checker path is not Core: ${path}`);
            }
            if (!existsSync(join(root, path)))
                fail(`checker path does not resolve: ${path}`);
        }
        for (const path of inventory.paths.filter((item) => (item.startsWith('scripts/') && item.endsWith('.ts')))) {
    ...
    results.run('CB5', 'adapter manifest shape', (fail) => {
        for (const [adapterId, paths] of Object.entries(manifest.files.adapter)) {
            ...
            for (const error of adapterShapeErrors(value))
                fail(`${adapterId}: ${error}`);
        ...
        try {
            ...
            fail(`adapter.schema.json is malformed: ${error instanceof Error ? error.message : String(error)}`);
        }
    ...
    results.run('CB8', 'foreign-adapter exclusion', (fail) => {
        ...
        for (const [adapterId, adapter] of adapters) {
            ...
            for (const foreignId of adapterIds.filter((id) => id !== adapterId)) {
                ...
                const foreignName = new RegExp(`(^|[^A-Za-z0-9])${foreignId.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}([^A-Za-z0-9]|$)`, 'i');
                ...
                for (const value of stringLeaves(adapter)) {
                    if (forbiddenPaths.some((token) => value.includes(token))
                        ...
                        fail(`${adapterId}: decoded adapter manifest names foreign adapter or host `
                            + foreignId);
                        break;
                    }
                }
        ...
        for (const target of manifest.bundle_targets) {
            ...
            for (const path of foreignPaths) {
                if (selected.has(path))
                    fail(`${target.id}: bundle includes foreign adapter path ${path}`);
            }
        }
    ...
    results.run('CB9', 'bundle digests and Core equality', (fail) => {
        ...
        for (const target of manifest.bundle_targets) {
            if (targetIds.has(target.id))
                fail(`duplicate bundle target id ${target.id}`);
            ...
            const plan = buildBundlePlan(root, manifest, target, adapter, options.bundleProvenance?.[target.id]);
...
function parseCli(args) {
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (arg === '--json')
            ...
            const value = args[index + 1];
            if (!value)
                options.error = '--preflight requires an adapter id';
            else {
...
function printHuman(report) {
    ...
    for (const adapter of report.adapters) {
        console.log(`PREFLIGHT ${adapter.id} ${adapter.preflight} lifecycle=${adapter.lifecycle}`
...
function main() {
    ...
    if (options.help) {
        console.log('Usage: node scripts/validate-core-boundary.ts '
