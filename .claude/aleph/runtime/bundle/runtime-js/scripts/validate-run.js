...
import { runK2 } from './lib/checks-k2.js';
import { runK3, runK4K5, runK6 } from './lib/checks-k3-k6.js';
import { parseFencedBlock } from './lib/markdown.js';
import { ResultCollector } from './lib/results.js';
import { loadRun } from './lib/run-model.js';
...
export function validateRun(options = {}) {
    ...
    if (kind === 'run') {
        ...
        if (model.evidenceDocument)
            runK3(results, model);
        if (model.cards.length > 0 || model.referentDocument)
            runK4K5(results, model);
        const reachedProjection = Boolean(model.manifest?.states.some((row) => (['PROJECTING', 'PROJECTION-ACCEPTED'].includes(row.values.state.trim()))));
        if (model.projections.length > 0 || reachedProjection)
            runK6(results, model);
    }
