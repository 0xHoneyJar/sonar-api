...
function validateUtf8(bytes, path) {
    try {
...
function assertEmptySourcesDirectory(path) {
    if (!existsSync(path))
        return;
...
export function snapshotCorpus(options) {
    if (!validRunId(options.runId))
        throw new Error(`invalid run ID: ${options.runId}`);
    if (options.inputs.length === 0)
        throw new Error('start requires at least one file or directory');
    ...
    const supported = new Set((options.supportedExtensions || DEFAULT_SUPPORTED_SOURCE_EXTENSIONS)
...
export function verifyCorpusSnapshot(runDir) {
    ...
    if (expected.size !== snapshot.files.length)
        throw new Error('corpus snapshot contains duplicate paths');
    ...
    if (digest !== snapshot.tree_digest)
        throw new Error('corpus tree digest mismatch');
    return snapshot;
}
function validateAuthorityResponse(snapshot, candidate) {
    ...
    const retained = snapshot.files.filter((source) => (rulings.get(source.source_id)?.decision === 'admit-exact-bytes'));
    const excluded = snapshot.files.filter((source) => (rulings.get(source.source_id)?.decision === 'exclude'));
    if (retained.length === 0)
        throw new Error('S0 authority excluded every staged source');
    return { response, retained, excluded };
}
export function planCorpusFreeze(runDir, candidate) {
    const root = resolve(runDir);
    const snapshot = verifyCorpusSnapshot(root);
    if (snapshot.status !== 'staged')
        throw new Error('corpus is already frozen');
...
function writeJsonExactOrFail(path, value, label) {
    if (existsSync(path)) {
...
export function applyCorpusFreeze(runDir, plan) {
    const root = resolve(runDir);
    ...
    writeJsonExactOrFail(join(root, 'control', 'gates', 'GATE-S0-response.json'), plan.response, 'S0 authority response');
...
export function freezeCorpus(runDir, candidate) {
