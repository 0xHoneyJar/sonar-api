...
import { chmodSync, closeSync, constants, existsSync, fchmodSync, fstatSync, fsyncSync, lstatSync, mkdirSync, openSync, readFileSync, readdirSync, renameSync, rmSync, writeFileSync, } from 'node:fs';
...
export function assertNoSymlinkComponents(root, target) {
    ...
    if (!existsSync(parent))
        throw new Error(`root does not exist: ${parent}`);
    if (lstatSync(parent).isSymbolicLink())
        throw new Error(`root is a symlink: ${parent}`);
...
export function readStableRegularFile(path) {
...
export function walkRegularFiles(root) {
    ...
    function visit(path) {
        ...
        if (stat.isSymbolicLink())
            throw new Error(`symlink inputs are unsupported: ${path}`);
        if (stat.isFile()) {
            files.push(path);
            return;
        }
    ...
    visit(absoluteRoot);
    return files.sort((left, right) => utf8Compare(relative(absoluteRoot, left), relative(absoluteRoot, right)));
}
...
export function writeFileAtomic(path, bytes, mode = 0o600) {
    const absolute = resolve(path);
    const directory = dirname(absolute);
    mkdirSync(directory, { recursive: true });
    assertNoSymlinkComponents(directory, absolute);
    temporaryCounter += 1;
    const temporary = join(directory, `.${basename(absolute)}.tmp-${String(process.pid)}-${String(temporaryCounter)}`);
...
export function copyExactFile(source, destination, mode = 0o600) {
...
export function digestTreeRecords(records) {
    ...
    for (const record of ordered) {
        ...
        if (seen.has(record.path))
            throw new Error(`duplicate tree record path: ${record.path}`);
...
export function makeTreeReadOnly(root) {
    ...
    function visit(path) {
        const stat = lstatSync(path);
        if (stat.isSymbolicLink())
            throw new Error(`cannot freeze a symlink: ${path}`);
        if (stat.isFile()) {
            chmodSync(path, 0o444);
            return;
        }
        if (!stat.isDirectory())
            throw new Error(`cannot freeze a non-file: ${path}`);
...
export function makeTreeOwnerWritable(root) {
    function visit(path) {
        const stat = lstatSync(path);
        if (stat.isSymbolicLink())
            throw new Error(`cannot thaw a symlink: ${path}`);
        if (stat.isFile()) {
            chmodSync(path, 0o600);
            return;
        }
        if (!stat.isDirectory())
            throw new Error(`cannot thaw a non-file: ${path}`);
...
export function nextDecimal(value) {
