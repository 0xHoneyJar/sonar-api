...
export interface VerifiedLoaBundle {
  root: string;
  lock: BundleLock;
  lockBytes: Buffer;
...
export interface LoadedCorePart {
  path: string;
  selector: string;
  bytes: Buffer;
  digest: string;
...
export interface LoadedOutputContract {
  path: string;
  selector: string;
  bytes: Buffer;
  digest: string;
...
function parseBundleLock(bytes: Buffer): BundleLock {
  let value: unknown;
  try {
  ...
  return value as BundleLock;
}

export function verifyAndLoadLoaBundle(bundleRoot: string): VerifiedLoaBundle {
  ...
  const files = new Map<string, BundleFileRecord>();
  const fileBytes = new Map<string, Buffer>();
  for (const file of [...lock.files].sort((left, right) => utf8Compare(left.path, right.path))) {
    if (files.has(file.path)) throw new Error(`duplicate locked bundle path: ${file.path}`);
...
export function readVerifiedBundleLock(bundle: VerifiedLoaBundle): Buffer {
...
export function lockedFileRecord(
  ...
  if (!record) throw new Error(`path is not present in the verified bundle: ${path}`);
...
export function readLockedFile(
  bundle: VerifiedLoaBundle,
  path: string,
  classification?: BundleFileClassification,
): Buffer {
...
export function extractMarkdownHeading(
  bytes: Buffer,
  heading: string,
  label = 'document',
): Buffer {
  ...
  if (start < 0) throw new Error(`${label} has no heading ${JSON.stringify(expected)}`);
  return bytes.subarray(start, end);
}

export function extractFirstFence(
  bytes: Buffer,
...
export function loadOutputContract(
  ...
  let example: unknown;
  try {
    example = JSON.parse(bytes.toString('utf8')) as unknown;
  } catch (error) {
