...
import {
  ...
  LOA_CLAUDE_CODE_PROBE_FORMAT,
  LOA_CLAUDE_CODE_RUNTIME_FORMAT,
  LOA_HOST_FORMAT,
  LOA_MODEL_SLOTS,
  LOA_REQUIRED_HOST_CAPABILITIES,
  type ClaudeCodeDispatchEvidence,
  type ClaudeCodeModelUsageEvidence,
  type ClaudeCodeProbeEvidence,
  type ClaudeCodeRuntimeAttestation,
  type ClaudeCodeUsageEvidence,
  type ExactModelIdentity,
  type HostExecutableIdentity,
  type JsonValue,
...
import { contractExemplarToJsonSchema } from './worker-return.ts';
...
const PROBE_OUTPUT: JsonValue = { attestation: 'ok' };
...
export interface AttestClaudeCodeHostOptions {
  profile: LoaProfile;
  modelId: string;
...
export interface ClaudeCodeWorkerInvocation {
...
export interface ClaudeCodeWorkerResult {
  ...
  structuredReturn: JsonValue;
  eventStream: Buffer;
  evidence: ClaudeCodeDispatchEvidence;
}

interface ParsedClaudeCodeStream {
  structuredOutput: JsonValue;
  evidence: Omit<
    ClaudeCodeProbeEvidence,
    'format' | 'slots' | 'effort' | 'event_stream'
  >;
}

interface StructuredInvocationResult {
  raw: Buffer;
  parsed: ParsedClaudeCodeStream;
  promptDigest: string;
  schemaDigest: string;
}
...
function decimalInteger(value: unknown, label: string, minimum = 0): number {
...
function decimalMoney(value: unknown, label: string): string {
...
export function isProviderPinnedClaudeModelId(value: string): boolean {
  const dated = /^claude-[a-z0-9]+-[1-9][0-9]*-[1-9][0-9]*-[0-9]{8}$/u;
  if (dated.test(value)) return true;
  const dateless = /^claude-[a-z0-9]+-([1-9][0-9]*)(?:-([1-9][0-9]*))?$/u.exec(value);
  if (!dateless) return false;
...
function resolveExecutable(input: string, label: string): string {
...
export function resolveClaudeCodeExecutable(input = 'claude'): string {
  return resolveExecutable(input, 'Claude Code executable');
}

export function resolveBubblewrapExecutable(input = 'bwrap'): string {
...
function readOnlyMounts(): Array<{ source: string; destination: string }> {
  return REQUIRED_READ_ONLY_MOUNTS.map((path) => {
    if (!existsSync(path)) throw new Error(`required sandbox mount is missing: ${path}`);
...
function hostBuildId(runtime: ClaudeCodeRuntimeAttestation): string {
  return sha256Digest(stableJsonBytes({
...
function minimalBedrockEnvironment(source: NodeJS.ProcessEnv = process.env): NodeJS.ProcessEnv {
  ...
  for (const name of BEDROCK_CREDENTIAL_ENVIRONMENT) {
    ...
    if (!value) throw new Error(`required Bedrock environment variable is missing: ${name}`);
...
function assertCurrentExecutable(
  executable: HostExecutableIdentity,
  label: string,
): void {
  if (!isRecord(executable)) throw new Error(`${label} executable identity is malformed`);
...
function sandboxArguments(
  runtime: ClaudeCodeRuntimeAttestation,
  workerRootInput: string,
  claudeArgs: string[],
): { workerRoot: string; args: string[] } {
  ...
  const expectedPolicy = sandboxPolicyDigest(
    runtime.claude.path,
    runtime.sandbox.read_only_mounts,
  );
...
function parseEventStream(raw: Buffer): Record<string, unknown>[] {
  ...
  return lines.map((line, index) => {
    ...
    try {
      ...
      throw new Error(
        `Claude Code event ${String(index + 1)} is invalid JSON: ${
...
function usageEvidence(value: unknown, label: string): ClaudeCodeUsageEvidence {
  if (!isRecord(value)) throw new Error(`${label} usage is missing`);
  return {
    ...
    cache_read_input_tokens: tokenString(
      value.cache_read_input_tokens,
      `${label}.cache_read_input_tokens`,
    ),
    cache_creation_input_tokens: tokenString(
...
function modelUsageEvidence(
  value: unknown,
  label: string,
): ClaudeCodeModelUsageEvidence {
  if (!isRecord(value)) throw new Error(`${label} model usage is missing`);
  return {
...
export function parseClaudeCodeStream(
  ...
  if (init.type !== 'system'
    ...
    || !sameStrings(init.tools as string[], [
      'StructuredOutput',
    ])
  ...
  if (!UUID.test(init.session_id)) throw new Error('Claude Code session ID is malformed');
  const sessionId = init.session_id;
  let structuredToolId: string | null = null;
  let structuredToolInput: JsonValue = null;
  ...
  for (const [index, event] of events.entries()) {
    ...
    if (event.type === 'assistant') {
      ...
      for (const block of event.message.content) {
        if (!isRecord(block)) throw new Error('Claude Code assistant content block is malformed');
        if (block.type === 'text') {
        ...
        structuredToolInput = block.input as JsonValue;
  ...
  try {
    resultText = JSON.parse(result.result) as unknown;
  } catch {
  ...
  const modelUsage = modelUsageEvidence(
    result.modelUsage[expectedModel],
    `Claude Code model ${expectedModel}`,
  );
...
function invokeStructured(
  ...
  const maxOutputBytes = decimalInteger(
    runtime.dispatch.max_output_bytes,
    'dispatch.max_output_bytes',
    1024,
  );
  const timeoutMs = decimalInteger(runtime.dispatch.timeout_ms, 'dispatch.timeout_ms', 1000);
  const maxBudgetUsd = decimalMoney(
    runtime.dispatch.max_budget_usd,
    'dispatch.max_budget_usd',
  );
  ...
  assertCurrentExecutable(runtime.claude, 'Claude Code');
  assertCurrentExecutable(runtime.sandbox, 'bubblewrap');
  if (result.error) throw new Error(`Claude Code worker process failed: ${result.error.message}`);
...
export function buildClaudeCodeWorkerPrompt(
  invocation: ClaudeCodeWorkerInvocation,
): Buffer {
  ...
  for (const [index, part] of request.core_parts.entries()) {
    ...
    chunks.push(
      `BEGIN CORE PART ${String(index + 1)} path=${JSON.stringify(part.path)} selector=${JSON.stringify(part.selector)} bytes=${String(bytes.byteLength)} digest=${part.digest}`,
      decodeUtf8(bytes, `Core prompt part ${part.materialized_path}`),
      `END CORE PART ${String(index + 1)}`,
      '',
    );
  }
  chunks.push(
  ...
  for (const [index, attachment] of request.allowlist.entries()) {
    ...
    chunks.push(
      `BEGIN ALLOWLISTED FILE ${String(index + 1)} run_path=${JSON.stringify(attachment.run_path)} bytes=${String(bytes.byteLength)} digest=${attachment.digest}`,
      decodeUtf8(bytes, `allowlisted worker file ${attachment.run_path}`),
      `END ALLOWLISTED FILE ${String(index + 1)}`,
      '',
    );
  }
...
function modelMechanics(
  profile: LoaProfile,
  slot: LoaModelSlot,
): Omit<ExactModelIdentity, 'provider' | 'model_id' | 'resolved_version' | 'identity_kind' | 'immutable' | 'fallback'> {
  const mappings = Object.values(profile.role_mappings).filter(
    (mapping) => mapping.model_slot === slot,
  );
  if (mappings.length === 0) throw new Error(`profile has no role for model slot ${slot}`);
...
function validateRuntimeShape(runtime: ClaudeCodeRuntimeAttestation): void {
  ...
  assertCurrentExecutable(runtime.claude as unknown as HostExecutableIdentity, 'Claude Code');
...
export function validateClaudeCodeHostCapabilities(
  ...
  const covered = new Set<LoaModelSlot>();
  for (const probe of runtime.probes) {
    ...
    const expectedEvidence: ClaudeCodeProbeEvidence = {
    ...
    for (const slot of probe.slots) {
      if (!(LOA_MODEL_SLOTS as readonly string[]).includes(slot) || covered.has(slot)) {
...
export function attestClaudeCodeHost(
  options: AttestClaudeCodeHostOptions,
): LoaHostCapabilities {
  ...
  const claudeVersionOutput = versionOutput(
    claudePath,
    ['--version'],
    spawn,
    'Claude Code',
  );
  const claudeMatch = /^([0-9]+\.[0-9]+\.[0-9]+) \(Claude Code\)$/u.exec(
    claudeVersionOutput,
  );
  if (!claudeMatch) throw new Error('Claude Code version output is unsupported');
  const sandboxVersionOutput = versionOutput(
    sandboxPath,
    ['--version'],
    spawn,
    'bubblewrap',
  );
  const sandboxMatch = /^bubblewrap ([0-9]+\.[0-9]+\.[0-9]+)$/u.exec(
    sandboxVersionOutput,
  );
  if (!sandboxMatch) throw new Error('bubblewrap version output is unsupported');
  const mounts = readOnlyMounts();
  const runtime: ClaudeCodeRuntimeAttestation = {
  ...
  const groups = new Map<string, { effort: string; slots: LoaModelSlot[] }>();
  ...
  for (const group of groups.values()) {
    ...
    try {
      const invocation = invokeStructured(
        runtime,
        workerRoot,
        options.modelId,
        group.effort,
        Buffer.from('Return the required attestation value through StructuredOutput.\n', 'utf8'),
        PROBE_SCHEMA,
        spawn,
        options.environment,
      );
  ...
  const host: LoaHostCapabilities = {
    ...
    capabilities: Object.fromEntries(
      LOA_REQUIRED_HOST_CAPABILITIES.map((capability) => [capability, true]),
    ) as LoaHostCapabilities['capabilities'],
    models,
    runtime,
    simulation: null,
  };
...
export function invokeClaudeCodeWorker(
  invocation: ClaudeCodeWorkerInvocation,
  host: LoaHostCapabilities,
  spawn: ClaudeCodeSpawn = defaultSpawn,
  environment: NodeJS.ProcessEnv = process.env,
  ...
  try {
    exemplar = JSON.parse(contractBytes.toString('utf8')) as unknown;
  } catch (error) {
    throw new Error(
      `Claude Code worker output contract is invalid JSON: ${
        error instanceof Error ? error.message : String(error)
      }`,
    );
  }
  ...
  const completed = invokeStructured(
    host.runtime,
  ...
  const evidence: ClaudeCodeDispatchEvidence = {
