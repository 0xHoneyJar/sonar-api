...
interface RoleSpec {
  path: string;
  heading: string;
  stages: readonly CoreStage[];
  verifierFrame?: boolean;
}

const ROLE_SPECS: Partial<Record<LoaRoleId, RoleSpec>> = {
  'intake-clerk': {
...
const STAGE_HEADINGS: Record<CoreStage, string> = {
...
export interface AssembleWorkerBundleOptions {
...
export interface AssembledWorkerBundle {
  root: string;
  request: WorkerRequest;
}

function workerBundleDigest(root: string, request: WorkerRequest): string {
  const contentRecords = inventoryTree(root).filter((file) => file.path !== 'request.json');
  const requestProjection: WorkerRequest = {
    ...request,
    bundle_digest: '',
  };
...
function validateTaskLine(value: string): string {
...
function exactNonemptyContextId(value: unknown): value is string {
...
function assertDispatchableRoleStage(role: LoaRoleId, stage: CoreStage): void {
  ...
  if (verifier) {
    if (!verifier.stages.includes(stage)) {
      throw new Error(
        `verifier role ${role} cannot run at ${stage}; canonical stages are ${verifier.stages.join(', ')}`,
      );
    }
    return;
  }
  const roleSpec = ROLE_SPECS[role];
  if (!roleSpec) throw new Error(`role ${role} is not a dispatchable worker role`);
  if (!roleSpec.stages.includes(stage)) throw new Error(`role ${role} cannot run at ${stage}`);
}

function runBundleIdentity(state: LoaRunState): unknown {
  return {
    bundle: {
...
function selectedBundleIdentity(bundle: VerifiedLoaBundle): unknown {
  return {
...
function assertPinnedRunAndModel(
  runDir: string,
  runId: string,
  role: LoaRoleId,
  modelIdentity: ExactModelIdentity,
  ...
  if (!stableJsonBytes(selectedBundleIdentity(bundle)).equals(
    stableJsonBytes(runBundleIdentity(state)),
  )) {
...
function assertWorkerAttachmentPath(path: string): void {
...
function roleParts(
  ...
  if (verifierSpec) {
    const frame = loadCorePart(
      bundle,
      'docs/architecture/prompts/verifier-lenses.md',
      'fence:Common verifier frame (verbatim, after the common preamble)',
    );
    const lens = loadCorePart(
      bundle,
      'docs/architecture/prompts/verifier-lenses.md',
...
function blindPolicySlice(part: LoadedCorePart): {
  start: number;
  end: number;
  bytes: Buffer;
} {
  ...
  if (start < 0) throw new Error(`Core role ${part.path} ${part.selector} omits Bundle/Shown`);
  const withholdStart = part.bytes.indexOf(
    bundleStart >= 0 ? withholdMarker : withheldMarker,
    start,
  );
  if (withholdStart < 0) {
  ...
  if (bytes.byteLength === 0) throw new Error('Core blind-context directive is empty');
  return { start, end, bytes };
}

function blindPolicyFor(
  part: LoadedCorePart,
...
export function coreBlindPolicyReference(
  bundle: VerifiedLoaBundle,
  role: LoaRoleId,
  stage: CoreStage,
): string {
...
function canonicalRunFiles(runDir: string): string[] {
...
export function assembleWorkerBundle(
  options: AssembleWorkerBundleOptions,
): AssembledWorkerBundle {
  if (!(CORE_STAGES as readonly string[]).includes(options.stage)) {
  ...
  const root = resolve(
    options.outputDir || join(runDir, 'control', 'worker-bundles', options.callId),
  );
  assertPathWithin(runDir, root, 'worker bundle');
  if (existsSync(root)) throw new Error(`worker bundle already exists: ${root}`);
  ...
  for (const path of allowlist) {
    ...
    if (withheldNames.has(path)) throw new Error(`allowlist includes withheld path ${path}`);
  }
  ...
  if (requiredWithheld.length !== withheldNames.size
    ...
    throw new Error(
      'worker withheld selectors must exactly inventory every canonical run file outside the allowlist',
    );
  }
  ...
  try {
    ...
    const blindPolicy = blindPolicyFor(
      policyPart,
      coreParts[policyPartIndex].materialized_path,
    );
...
export function verifyWorkerBundle(root: string): WorkerRequest {
  const bundleRoot = resolve(root);
  const request = readJsonFile(join(bundleRoot, 'request.json')) as WorkerRequest;
  ...
  const policyPart = request.core_parts.find((part) => (
    part.materialized_path === policy.core_part_path
    && part.path === policy.core_path
    && part.selector === policy.selector
  ));
  if (!policyPart) throw new Error('worker blind policy does not name a sealed Core part');
