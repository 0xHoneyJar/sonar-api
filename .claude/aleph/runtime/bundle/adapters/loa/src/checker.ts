...
interface CheckerTransaction {
  format: typeof CHECKER_TRANSACTION_FORMAT;
  status: 'prepared' | 'committed';
  check_id: string;
  record_path: string;
  record_digest: string;
  record: CheckerRecord;
  kernel_report_path: string;
  kernel_report_digest: string;
  kernel_report: string;
  prepared_at: string;
  committed_at: string | null;
}

export type CheckerSpawn = (
  executable: string,
  args: string[],
  cwd: string,
) => SpawnSyncReturns<string>;

export interface InvokePinnedCheckerOptions {
  runDir: string;
  clock?: Clock;
  spawn?: CheckerSpawn;
...
export interface PinnedCheckerResult {
  record: CheckerRecord;
  recordPath: string;
  kernelReportPath: string;
}
...
function materializePinnedChecker(
  runDir: string,
  bundle: ReturnType<typeof verifyAndLoadLoaBundle>,
): { root: string; checkerPath: string } {
...
function nextCheckNumber(runDir: string): string {
  ...
  if (existsSync(verificationDirectory)) {
    indexes.push(...readdirSync(verificationDirectory).flatMap((name) => {
      if (name === 'kernel-report.md') return [1];
...
function isCanonicalCheckNumber(value: string): boolean {
...
function renderKernelReport(
  bundleRoot: string,
  record: CheckerRecord,
): string {
  const bundle = verifyAndLoadLoaBundle(bundleRoot);
  const templateFile = readLockedFile(
    bundle,
    'docs/architecture/templates/01-run-control.md',
    'core',
  );
  const section = extractMarkdownHeading(
    templateFile,
  ...
  if (start < 0 || end <= start) throw new Error('Core kernel-report template is malformed');
  let report = section
...
function kernelReportRelativePath(number: string): string {
...
function transactionRelativePath(number: string): string {
  return `control/transactions/TXN-checker-${number}.json`;
}

function validateCheckerTransaction(
  runDir: string,
  name: string,
  value: unknown,
  expectedRunId: string,
  expectedCheckerDigest: string,
  ...
  const record = value.record as unknown as CheckerRecord;
  ...
  void runDir;
  return value as unknown as CheckerTransaction;
}

function writeOrVerifyExact(path: string, bytes: Buffer, label: string): void {
  if (existsSync(path)) {
...
function applyCheckerTransaction(
  runDir: string,
  transactionPath: string,
  transaction: CheckerTransaction,
  committedAt: string,
): CheckerTransaction {
  ...
  writeOrVerifyExact(
    recordPath,
    stableJsonBytes(transaction.record),
    `${transaction.check_id} record`,
  );
  writeOrVerifyExact(
    reportPath,
    Buffer.from(transaction.kernel_report, 'utf8'),
    `${transaction.check_id} kernel report`,
  );
  ...
  const committed: CheckerTransaction = {
    ...transaction,
    status: 'committed',
...
function recoverCheckerTransactionsUnlocked(
  runDir: string,
  recoveredAt: string,
  expectedRunId: string,
  expectedCheckerDigest: string,
  ...
  for (const name of readdirSync(transactionRoot).sort()) {
    if (!/^TXN-checker-\d+\.json$/u.test(name)) continue;
    ...
    const transaction = validateCheckerTransaction(
      runDir,
      name,
      value,
      expectedRunId,
...
export function invokePinnedChecker(
  options: InvokePinnedCheckerOptions,
): PinnedCheckerResult {
  ...
  withCheckerLock(runDir, invokedAt, () => recoverCheckerTransactionsUnlocked(
    runDir,
    invokedAt,
    state.run_id,
    bundle.lock.checker_digest,
    bundle.lock.bundle.digest,
  ));
  ...
  const result = (options.spawn || defaultSpawn)(
    process.execPath,
    args,
    checkerInvocation.root,
  );
  ...
  return withCheckerLock(runDir, invokedAt, () => {
    recoverCheckerTransactionsUnlocked(
      runDir,
      invokedAt,
      state.run_id,
      bundle.lock.checker_digest,
    ...
    const transaction: CheckerTransaction = {
