...
import { verifyCorpusSnapshot } from './intake.ts';
import {
...
export function runtimeSnapshotPath(runDir: string): string {
  return join(resolve(runDir), RUNTIME_SNAPSHOT_PATH);
}

export interface InitializeRunControlOptions {
  loaRoot: string;
  runDir: string;
  runId: string;
  lock: BundleLock;
  originalLockBytes: Buffer;
...
interface DurableProcessLockOwner {
  format: string;
  pid: string;
  acquired_at: string;
  nonce: string;
...
export interface DurableProcessLockOptions {
  format: string;
  label: string;
  acquiredAt: string;
}
...
function parseDurableProcessLock(
  bytes: Buffer,
  expectedFormat: string,
): DurableProcessLockOwner | null {
  let value: unknown;
  try {
  ...
  return value as unknown as DurableProcessLockOwner;
}

function partialLockPid(bytes: Buffer): number | null {
...
export function acquireDurableProcessLock(
  ...
  const readOwner = (path: string): {
    bytes: Buffer;
    owner: DurableProcessLockOwner | null;
  } => {
  ...
  const recoveryCandidates = (): Array<{
    ...
    return candidates.sort((left, right) => {
      if (left.modifiedNs < right.modifiedNs) return -1;
      if (left.modifiedNs > right.modifiedNs) return 1;
      return utf8Compare(left.path, right.path);
    });
  };

  const acquireRecoveryOwnership = (): void => {
    ...
    let observedRecoveryOwner: Buffer | null = null;
    if (existsSync(recoveryOwnerPath)) {
      ...
      if (processIsAlive(Number(record.owner.pid))) {
        throw new Error(
          `${options.label} recovery is already active under pid ${record.owner.pid}`,
        );
      }
    }
    ...
    if (!winner || winner.path !== recoveryContenderPath) {
      throw new Error(
        `${options.label} recovery is already active${winner ? ` under pid ${winner.owner.pid}` : ''}`,
      );
    }
...
function mapModels(
  profile: LoaProfile,
  host: LoaHostCapabilities,
): Record<LoaRoleId, ExactModelIdentity> {
  return Object.fromEntries(Object.entries(profile.role_mappings).map(([role, mapping]) => {
    ...
    if (!identity) throw new Error(`host did not resolve model slot ${mapping.model_slot}`);
    return [role, identity];
  })) as Record<LoaRoleId, ExactModelIdentity>;
}

export function runDirectory(loaRoot: string, runId: string): string {
...
export function createRunDirectory(loaRoot: string, runId: string): string {
  const root = runDirectory(loaRoot, runId);
  if (existsSync(root)) throw new Error(`run already exists: ${runId}`);
...
export function listRunIds(loaRoot: string): string[] {
  const root = resolve(loaRoot, LOA_RUN_ROOT);
...
export function stateCheckpointDigest(state: LoaRunState): string {
  const projection: LoaRunState = structuredClone(state);
  projection.execution.resume.checkpoint_digest = '';
  return sha256Digest(stableJsonBytes(projection));
}

export function assertLoaRunState(value: unknown): asserts value is LoaRunState {
  if (!isRecord(value)
  ...
  if (typeof execution.core_state !== 'string'
    || !(CORE_RUN_STATES as readonly string[]).includes(execution.core_state)
    || typeof execution.stage !== 'string'
    || !(CORE_STAGES as readonly string[]).includes(execution.stage)
...
export function readRunState(runDir: string): LoaRunState {
...
export function writeRunState(runDir: string, state: LoaRunState): void {
...
export function initializeRunControl(options: InitializeRunControlOptions): LoaRunState {
...
export interface HumanAuthorityDecision {
  ...
  recordedAt: string;
  simulation: null | { kind: 'fixture-simulated' };
  response: JsonValue;
  approvedState?: CoreRunState;
}

function validGateId(value: string): boolean {
  return /^GATE-[A-Za-z0-9]+(?:[._-][A-Za-z0-9]+)*$/u.test(value);
}

function exactAuthorityIdentity(value: unknown): value is string {
...
} as const satisfies Record<string, readonly CoreStage[]>;

type GenericGateType = keyof typeof GATE_STAGE_RULES;

interface AuthorityLockOwner {
  format: typeof AUTHORITY_LOCK_FORMAT;
  pid: string;
  acquired_at: string;
  nonce: string;
}
...
interface AuthorityTransaction {
  format: typeof AUTHORITY_TRANSACTION_FORMAT;
  operation: 'open-gate' | 'record-decision';
...
function defaultAuthorityClock(): Clock {
...
function assertGateTypeStage(gateType: string, stage: CoreStage): asserts gateType is GenericGateType {
  const stages = GATE_STAGE_RULES[gateType as GenericGateType];
  if (!stages || !(stages as readonly string[]).includes(stage)) {
...
function assertGatePrerequisites(
  current: LoaRunState,
  gateType: GenericGateType,
  stage: CoreStage,
): void {
  if (current.execution.stage !== stage) {
    throw new Error(
      `human authority gate cannot jump from Core stage ${current.execution.stage} to ${stage}`,
    );
  }
  ...
  if (gateType === 'projection-commission'
    ...
    throw new Error(
      'P1 projection commission requires Core state ACCEPTED or PROJECTION-ACCEPTED',
    );
  }
...
function authorityTransactionPath(
  runDir: string,
  operation: AuthorityTransaction['operation'],
  gateId: string,
): string {
...
function parseAuthorityTransaction(path: string): AuthorityTransaction {
  const value = readJsonFile(path);
  ...
  const transaction = value as unknown as AuthorityTransaction;
...
function applyAuthorityTransaction(
  runDir: string,
...
function recoverPendingAuthorityTransactionsUnlocked(
  runDir: string,
  recoveredAt: string,
): AuthorityRecoveryResult {
  const result: AuthorityRecoveryResult = { committed: [], alreadyCommitted: [] };
  const root = join(runDir, 'control', 'transactions');
  if (!existsSync(root)) return result;
  for (const name of readdirSync(root).sort(utf8Compare)) {
    if (!/^TXN-authority-(?:open|decision)-GATE-[A-Za-z0-9._-]+\.json$/u.test(name)) continue;
    const path = join(root, name);
...
export function recoverPendingAuthorityTransactions(
  runDir: string,
  clock: Clock = defaultAuthorityClock(),
): AuthorityRecoveryResult {
...
function prepareAndCommitAuthorityTransaction(
  runDir: string,
  transaction: AuthorityTransaction,
): LoaRunState {
...
export function openHumanAuthorityGate(
  runDir: string,
  options: OpenHumanAuthorityGateOptions,
): LoaRunState {
  if (!validGateId(options.gateId)
    || !(CORE_STAGES as readonly string[]).includes(options.stage)
...
function validateDecisionState(
  ...
  if (decision.approvedState !== undefined
    && !(CORE_RUN_STATES as readonly string[]).includes(decision.approvedState)) {
...
export function recordHumanAuthorityDecision(
  runDir: string,
  decision: HumanAuthorityDecision,
): LoaRunState {
  ...
  try {
    ...
    if (!existsSync(requestPath)) throw new Error('human authority gate request disappeared');
    ...
    const after = transitionedState(current, decision.recordedAt, (draft) => {
      if (decision.simulation !== null) draft.full_mode = 'fixture-simulated';
      if (!draft.execution.gate) throw new Error('human authority gate disappeared during update');
      ...
      if (decision.decision === 'approve') {
        ...
        if (decision.approvedState) draft.execution.core_state = decision.approvedState;
      } else {
        draft.execution.halt = {
