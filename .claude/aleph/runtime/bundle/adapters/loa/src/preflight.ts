...
import { verifyBundle } from '../../../scripts/assemble-bundles.ts';
import { validateCoreBoundary } from '../../../scripts/validate-core-boundary.ts';
import {
  LOA_ADAPTER_ID,
  LOA_BUNDLE_ID,
  LOA_HOST_FORMAT,
...
export interface LoaPreflightCheck {
  id: string;
  title: string;
  status: 'PASS' | 'FAIL';
  problems: string[];
...
export interface LoaPreflightReport {
  result: 'PASS' | 'FAIL';
  root: string;
  rootMode: 'source' | 'bundle' | 'unknown';
  capabilitiesPath: string | null;
  evidenceClass: 'runtime' | 'fixture-simulated' | 'unresolved';
  runtimeReady: boolean;
  checks: LoaPreflightCheck[];
}

export interface RunLoaPreflightOptions {
  root?: string;
  capabilities?: string;
}

interface BoundaryContext {
  mode: 'source' | 'bundle' | 'unknown';
  corePaths: Set<string>;
  adapterPaths: Set<string>;
  foreignAdapterIds: string[];
}
...
class CheckCollector {
  readonly checks: LoaPreflightCheck[] = [];

  run(id: string, title: string, operation: (fail: (message: string) => void) => void): void {
...
function existingRegularFile(root: string, path: string): boolean {
...
function immutableResolvedIdentity(value: unknown): value is string {
  return typeof value === 'string' && /^sha256:[0-9a-f]{64}$/.test(value);
}

function fallbackProblems(value: unknown, scope = 'value'): string[] {
  const problems: string[] = [];
...
function degradedModeProblems(value: unknown, scope = 'value'): string[] {
  const problems: string[] = [];
  if (Array.isArray(value)) {
    for (const [index, item] of value.entries()) {
...
function loadBoundaryContext(
  root: string,
  collector: CheckCollector,
): BoundaryContext {
  let context: BoundaryContext = {
    mode: 'unknown',
    corePaths: new Set<string>(),
    adapterPaths: new Set<string>(),
    foreignAdapterIds: [],
  };
  ...
  collector.run('LP1', 'verified source or immutable bundle root', (fail) => {
    ...
    if (existsSync(lockPath)) {
      ...
      if (verification.result !== 'PASS') {
        for (const error of verification.errors) fail(`bundle verification: ${error}`);
      }
...
function validateManifest(
  ...
  collector.run('LP2', 'implemented full-mode adapter manifest', (fail) => {
    ...
    if (!Array.isArray(value.entrypoints) || value.entrypoints.length === 0) {
      ...
      const command = value.entrypoints.find((entrypoint) => (
        isRecord(entrypoint) && entrypoint.id === '/loa-aleph'
      ));
      if (!isRecord(command)
    ...
    if (!Array.isArray(value.profiles) || value.profiles.length === 0) {
      ...
      const declared = value.profiles.find((profile) => (
        isRecord(profile) && profile.path === PROFILE_PATH
      ));
...
function validateAdapterInventory(
  root: string,
  boundary: BoundaryContext,
  collector: CheckCollector,
): void {
  collector.run('LP3', 'Loa ownership and generic foreign-adapter exclusion', (fail) => {
    for (const path of boundary.adapterPaths) {
      ...
      for (const foreignId of boundary.foreignAdapterIds) {
        ...
        if (token.test(text)) fail(`${path} names a foreign adapter or host`);
      }
...
function validateRequiredPaths(
  ...
  collector.run('LP4', 'required command, skill, profile, and runtime sources', (fail) => {
    for (const path of REQUIRED_ADAPTER_PATHS) {
      if (!boundary.adapterPaths.has(path)) fail(`required path is not Loa-owned: ${path}`);
      if (!existingRegularFile(root, path)) fail(`required path does not resolve: ${path}`);
    }
  });
}
...
function validateInstallationMap(
  ...
  collector.run('LP5', 'offline complete-bundle installation map', (fail) => {
    ...
    for (const exposure of value.exposures) {
      ...
      if (seenIds.has(exposure.id)) fail(`duplicate exposure id ${exposure.id}`);
    ...
    for (const expected of EXPECTED_EXPOSURES) {
      const match = value.exposures.find((exposure) => (
        isRecord(exposure) && exposure.id === expected.id
      ));
      if (!isRecord(match)
...
function validateCanonicalCore(
  root: string,
  boundary: BoundaryContext,
  ...
  collector.run('LP6', 'direct canonical Core contracts and checkers', (fail) => {
    for (const path of REQUIRED_CORE_PATHS) {
      if (!boundary.corePaths.has(path)) fail(`canonical Core path is not Core-owned: ${path}`);
      if (!existingRegularFile(root, path)) fail(`canonical Core path does not resolve: ${path}`);
    }
...
function validateProfile(
  ...
  collector.run('LP7', 'complete Loa profile and Core role coverage', (fail) => {
    ...
    if (!isRecord(roles) || !sameStrings(Object.keys(roles), ROLE_KEYS)) {
      ...
      for (const role of ROLE_KEYS) {
        ...
        if (typeof mapping.model_slot === 'string'
          && !MODEL_SLOT_KEYS.includes(mapping.model_slot as typeof MODEL_SLOT_KEYS[number])) {
    ...
    for (const problem of fallbackProblems(value, 'profile')) fail(problem);
    for (const problem of degradedModeProblems(value, 'profile')) fail(problem);
  });
  return profile;
}
...
function validateHostCapabilities(
  ...
  collector: CheckCollector,
): { evidenceClass: LoaPreflightReport['evidenceClass']; runtimeReady: boolean } {
  let evidenceClass: LoaPreflightReport['evidenceClass'] = 'unresolved';
  ...
  collector.run('LP8', 'resolved immutable host and model capabilities', (fail) => {
    ...
    if (value.host_format !== HOST_FORMAT) fail(`host format must be ${HOST_FORMAT}`);
    ...
    if (!isRecord(capabilities)
      ...
      for (const key of HOST_CAPABILITY_KEYS) {
        if (capabilities[key] !== true) fail(`host capability ${key} is unavailable`);
      }
    }
    ...
    if (profile && isRecord(profile.model_slots) && isRecord(models)) {
      for (const slot of Object.keys(profile.model_slots)) {
        if (!isRecord(models[slot])) fail(`profile model slot ${slot} is unresolved`);
      }
    }
    if (profile && isRecord(profile.role_mappings) && isRecord(models)) {
      ...
      for (const [role, rawMapping] of Object.entries(profile.role_mappings)) {
        if (!isRecord(rawMapping) || typeof rawMapping.model_slot !== 'string') continue;
        ...
        for (const [profileField, hostField] of mechanicFields) {
          if (rawMapping[profileField] !== model[hostField]) {
            fail(
              `role ${role} ${profileField}=${String(rawMapping[profileField])} does not match `
    ...
    for (const problem of fallbackProblems(value, 'host')) fail(problem);
    for (const problem of degradedModeProblems(value, 'host')) fail(problem);
  });
  return { evidenceClass, runtimeReady };
}

function validateNoSilentDowngrade(
  manifest: JsonRecord | null,
  profile: JsonRecord | null,
  hostState: { evidenceClass: LoaPreflightReport['evidenceClass']; runtimeReady: boolean },
  collector: CheckCollector,
): void {
  collector.run('LP9', 'fail-closed full-mode labeling', (fail) => {
    ...
    if (!profile) fail('profile is unavailable; no partial profile may be relabeled full mode');
...
export function runLoaPreflight(
  options: RunLoaPreflightOptions = {},
): LoaPreflightReport {
...
function parseCli(args: string[]): CliOptions {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === '--json') options.json = true;
      ...
      const value = args[index + 1];
      if (!value) options.error = '--capabilities requires a JSON file';
      else {
...
function printHuman(report: LoaPreflightReport): void {
...
function main(): void {
  ...
  if (options.help) {
    console.log(
      'Usage: node adapters/loa/src/preflight.ts '
