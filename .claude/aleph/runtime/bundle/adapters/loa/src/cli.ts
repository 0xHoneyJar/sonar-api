...
export interface LoaCommandOptions {
  loaRoot?: string;
  bundleRoot?: string;
  ...
  clock?: Clock;
  idSource?: IdSource;
  checkerSpawn?: CheckerSpawn;
}

interface ParsedCli {
  argv: string[];
  options: LoaCommandOptions;
  json: boolean;
...
function parseFileOpenGate(value: unknown): OpenHumanAuthorityGateOptions {
  ...
  return value as unknown as OpenHumanAuthorityGateOptions;
}

function parseFileHumanAuthorityDecision(value: unknown): HumanAuthorityDecision {
  if (!exactKeys(value, [
    'gateId',
    'authorityIdentity',
    'decision',
  ...
  return value as unknown as HumanAuthorityDecision;
}
...
function result(
  ...
  status: LoaCommandResult['result'],
  fields: Partial<Omit<LoaCommandResult, 'format' | 'command' | 'result'>> = {},
): LoaCommandResult {
  return {
...
function stripTemplateComments(value: string): string {
...
function insertTableRow(document: string, heading: string, row: string): string {
  const rendered = document.endsWith('\n') ? document : `${document}\n`;
  const headingAt = rendered.indexOf(heading);
  if (headingAt < 0) throw new Error(`rendered Core template omits ${heading}`);
...
function renderRunManifest(
  bundle: VerifiedLoaBundle,
  state: LoaRunState,
  now: string,
): string {
  let manifest = stripTemplateComments(templateBlock(
    bundle,
    'docs/architecture/templates/01-run-control.md',
    'T1.1 Run manifest → `runs/<run-id>/run-manifest.md`',
  )).replaceAll('⟨RUN-slug⟩', state.run_id);
  ...
  manifest = manifest.split('\n').map((line) => {
    ...
    if (!match || fields[match[1]] === undefined) return line;
  ...
  const budgetMap = stableJson(Object.fromEntries(Object.entries(identity.models).map(
    ([role, model]) => [role, model.budget],
  )));
  manifest = manifest.split('\n').map((line) => {
    if (/^\| model_ids \(per role/u.test(line)) return `| model_ids (per role, exact strings; or "human") | ${safeTable(mappings)} |`;
    if (/^\| adapter profile ID/u.test(line)) return `| adapter profile ID + digest | ${safeTable(profileValue)} |`;
    if (/^\| model\/context\/effort mapping/u.test(line)) return `| model/context/effort mapping actually used | ${safeTable(mappings)} |`;
    if (/^\| profile deviations/u.test(line)) return `| profile deviations | ${state.full_mode === 'fixture-simulated' ? 'fixture-simulated; not validation or acceptance evidence' : 'none'} |`;
    if (/^\| fan-out limits/u.test(line)) return '| fan-out limits | pinned by host profile and per-stage dispatch |';
    if (/^\| budgets granted/u.test(line)) return `| budgets granted (per stage, tokens) | ${safeTable(budgetMap)} |`;
    if (/^\| 1 \| DRAFT/u.test(line)) return `| 1 | DRAFT | ${now} | loa-orchestrator | run created; S0 authority gate pending |`;
    if (line.includes('⟨approved/…⟩') || line.includes('| ⟨none | list⟩ |')) return '';
    return line;
  }).join('\n');
  if (state.full_mode === 'fixture-simulated') {
    manifest = insertTableRow(
      ...
      `| fixture-simulated host | structural implementation fixture only; not replay, validation, acceptance, or sanction evidence | ${now.slice(0, 10)} |`,
    );
  }
...
function renderRunLog(
  ...
  const template = stripTemplateComments(templateBlock(
    bundle,
    'docs/architecture/templates/01-run-control.md',
...
function renderCorpusManifest(
  bundle: VerifiedLoaBundle,
  state: LoaRunState,
  files: ReturnType<typeof snapshotCorpus>['files'],
): string {
  let manifest = stripTemplateComments(templateBlock(
    bundle,
  ...
  for (const file of files) {
    manifest = insertTableRow(
      manifest,
      '## Source inventory',
      `| ${file.source_id} | design-note | ${safeTable(file.frozen_path.replace(/^corpus\//u, ''))} | ${file.scheme} | ${file.digest} | not-recorded | unverifiable | pending-authority | staged exact bytes; admission pending S0 authority |`,
    );
  }
  return manifest.endsWith('\n') ? manifest : `${manifest}\n`;
}

function renderFrozenCorpusManifest(
  bundle: VerifiedLoaBundle,
  state: LoaRunState,
  staged: ReturnType<typeof verifyCorpusSnapshot>,
  frozen: CorpusFreezePlan['frozen'],
  ...
  if (schemes.has('text-lines')) {
    manifest = insertTableRow(
      ...
      '| text-lines | UTF-8 text files | `L⟨start⟩-L⟨end⟩` of the frozen file |',
    );
  }
  ...
  for (const file of frozen.files) {
    ...
    manifest = insertTableRow(
      manifest,
      '## Source inventory',
      `| ${file.source_id} | design-note | ${safeTable(file.frozen_path.replace(/^corpus\//u, ''))} | ${file.scheme} | ${file.digest} | not-recorded | unverifiable | ${ruling.labels.join(',')} | admitted as exact bytes by S0 human authority; see control/gates/GATE-S0-response.json |`,
    );
  }
  ...
  for (const sourceId of response.exclusions) {
    ...
    if (!file) throw new Error(`excluded source disappeared from staged inventory: ${sourceId}`);
...
function writeCanonicalDraft(
  bundle: VerifiedLoaBundle,
  runDir: string,
  state: LoaRunState,
  corpus: ReturnType<typeof snapshotCorpus>,
  now: string,
): void {
  ...
  writeFileAtomic(join(runDir, 'corpus', 'manifest.md'), renderCorpusManifest(
    bundle,
    state,
    corpus.files,
  ));
...
function runtimeDetails(state: LoaRunState): Record<string, JsonValue> {
  return {
...
export function startLoaRun(
  ...
  const capabilitiesPath = resolve(
    loaRoot,
    options.capabilitiesPath || DEFAULT_CAPABILITIES_PATH,
  );
  let runDir = '';
  try {
...
function stateSummary(state: LoaRunState): JsonValue {
...
export function validateLoaRun(
  runId: string,
  options: LoaCommandOptions = {},
): LoaCommandResult {
  const loaRoot = resolve(options.loaRoot || process.cwd());
...
function renderUpdatedManifestForS0(
  runDir: string,
  response: S0AuthorityResponse,
  state: LoaRunState,
): { runManifest: string; runLog: string } {
  const path = join(runDir, 'run-manifest.md');
  let manifest = readFileSync(path, 'utf8');
  manifest = manifest.replace(
    /- declared_scope: >\s*\n\s*PENDING S0 HUMAN AUTHORITY/u,
  ...
  manifest = insertTableRow(
    manifest,
    '## State log',
    `| ${state.execution.resume.sequence} | CORPUS-FROZEN | ${response.recorded_at} | ${safeTable(response.authority.identity)} | S0 authority approved exact frozen corpus |`,
  );
  manifest = insertTableRow(
    manifest,
    '## Authority sign-offs',
    `| S0 corpus scope + sensitivity | approved | ${safeTable(response.authority.identity)} | ${response.recorded_at.slice(0, 10)} | control/gates/GATE-S0-response.json |`,
  );
  if (response.simulation && !manifest.includes('fixture-simulated authority response')) {
    manifest = insertTableRow(
      manifest,
      '## Unvalidated-machinery notices',
      `| fixture-simulated authority response | structural fixture only; not acceptance evidence | ${response.recorded_at.slice(0, 10)} |`,
    );
  }
...
interface S0FreezeTransaction {
  format: typeof S0_TRANSACTION_FORMAT;
  status: 'prepared' | 'committed';
  run_id: string;
  prepared_at: string;
  state_before_checkpoint: string;
  plan: CorpusFreezePlan;
  state_after: LoaRunState;
  files_after: S0TransactionFiles;
  payload_digest: string;
  committed_at?: string;
}

function s0TransactionPayload(transaction: Omit<S0FreezeTransaction, 'payload_digest' | 'committed_at' | 'status'>): string {
  return sha256Digest(stableJsonBytes(transaction));
}

function acquireS0TransactionLock(
  runDir: string,
  acquiredAt: string,
...
function parseS0Transaction(path: string): S0FreezeTransaction {
  ...
  const transaction = value as unknown as S0FreezeTransaction;
  const payload = {
...
function applyS0Transaction(
  runDir: string,
  transactionPath: string,
  transaction: S0FreezeTransaction,
  committedAt: string,
): LoaRunState {
...
export function recoverPendingS0Transaction(
  runDir: string,
  ...
  try {
    return applyS0Transaction(
      root,
      transactionPath,
      parseS0Transaction(transactionPath),
      recoveredAt,
    );
  } finally {
    release();
...
export function recordS0AuthorityResponse(
  runId: string,
  response: S0AuthorityResponse,
  options: LoaCommandOptions = {},
): LoaCommandResult {
  ...
  try {
    const runDir = runDirectory(loaRoot, runId);
    release = acquireS0TransactionLock(
      runDir,
    ...
    if (existsSync(transactionPath)) throw new Error('S0 freeze transaction already exists');
    const plan = planCorpusFreeze(runDir, response);
    const state = structuredClone(prior);
    if (response.simulation) state.full_mode = 'fixture-simulated';
    state.corpus = {
      state: 'frozen',
    ...
    if (!state.execution.gate) throw new Error('S0 gate disappeared during update');
    ...
    const pinnedBundle = verifyAndLoadLoaBundle(runtime.bundle.root);
    const filesAfter: S0TransactionFiles = {
      run_manifest: updated.runManifest,
      run_log: updated.runLog,
      corpus_manifest: renderFrozenCorpusManifest(
        pinnedBundle,
        state,
        plan.staged,
        plan.frozen,
...
export function openGenericHumanAuthorityGate(
  runId: string,
  gate: OpenHumanAuthorityGateOptions,
  options: LoaCommandOptions = {},
): LoaCommandResult {
  const loaRoot = resolve(options.loaRoot || process.cwd());
  try {
...
export function recordGenericHumanAuthorityResponse(
  runId: string,
  decision: HumanAuthorityDecision,
  options: LoaCommandOptions = {},
): LoaCommandResult {
  const loaRoot = resolve(options.loaRoot || process.cwd());
  try {
...
function recordPersistedAuthorityResponse(
  runId: string,
  response: unknown,
  options: LoaCommandOptions,
): LoaCommandResult {
  ...
  try {
    ...
    if (state.execution.stage === 'S0'
      ...
      return recordS0AuthorityResponse(runId, response as S0AuthorityResponse, options);
    }
    return recordGenericHumanAuthorityResponse(
      runId,
      parseFileHumanAuthorityDecision(response),
      options,
    );
  } catch (error) {
    return result('resume', 'FAIL', {
...
export function dispatchLoaCommand(
  argv: string[],
  options: LoaCommandOptions = {},
): LoaCommandResult {
  ...
  if (command === 'status') {
    if (argv.length > 2) return result('status', 'FAIL', { errors: ['status accepts at most one RUN-id'] });
    return statusLoaRun(argv[1], options);
  }
  if (command === 'resume') {
    if (argv.length !== 2) return result('resume', 'FAIL', { errors: ['resume requires exactly one RUN-id'] });
    return resumeLoaRun(argv[1], options);
  }
  if (command === 'validate') {
    if (argv.length !== 2) return result('validate', 'FAIL', { errors: ['validate requires exactly one RUN-id'] });
...
function parseCli(argv: string[]): ParsedCli {
  const options: LoaCommandOptions = {};
  ...
  for (let index = 0; index < argv.length; index++) {
    ...
    else if (arg === '--root') options.loaRoot = argv[++index];
    else if (arg === '--capabilities') options.capabilitiesPath = argv[++index];
...
export function runLoaCli(argv = process.argv.slice(2)): number {
  ...
  if (parsed.authorityResponsePath && parsed.openGatePath) {
    ...
    if (!runId || parsed.argv.length !== 1) {
      ...
      try {
        commandResult = recordPersistedAuthorityResponse(
          runId,
          readJsonFile(parsed.authorityResponsePath),
          parsed.options,
        );
      } catch (error) {
        commandResult = result('resume', 'FAIL', {
    ...
    if (!runId || parsed.argv.length !== 1) {
      ...
      try {
        commandResult = openGenericHumanAuthorityGate(
          runId,
          parseFileOpenGate(readJsonFile(parsed.openGatePath)),
          parsed.options,
        );
      } catch (error) {
        commandResult = result('resume', 'FAIL', {
  ...
  if (parsed.json) {
    ...
    if (commandResult.state) console.log(`STATE ${commandResult.state} STAGE ${commandResult.stage}`);
    for (const error of commandResult.errors) console.error(`ERROR ${error}`);
    if (commandResult.gate) console.log(`GATE ${commandResult.gate.id} ${commandResult.gate.status}`);
  }
