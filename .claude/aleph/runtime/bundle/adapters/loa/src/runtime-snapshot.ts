...
import {
  ...
  LOA_ROLE_IDS,
  LOA_RUNTIME_SNAPSHOT_FORMAT,
  type ExactModelIdentity,
  type LoaHostCapabilities,
  type LoaModelSlot,
  type LoaProfile,
  type LoaRequiredHostCapability,
  type LoaRoleId,
...
import type { VerifiedLoaBundle } from './core-loader.ts';
import {
  readLockedFile,
...
export interface CaptureRuntimeSnapshotOptions {
  runId: string;
...
function exactImmutableLabel(value: unknown, label: string): asserts value is string {
  if (!nonemptyString(value) || MUTABLE_ALIAS.test(value)) {
    throw new Error(`${label} is empty, mutable, or aliased`);
  }
}

function contentAddressedIdentity(value: unknown, label: string): asserts value is string {
...
function parseRoleMapping(value: unknown, role: string): LoaProfile['role_mappings'][LoaRoleId] {
  const keys = [
    'model_slot',
    'effort',
    'context_policy',
  ...
  if (!exactKeys(value, keys)) throw new Error(`profile role ${role} fields are malformed`);
  const record = value as Record<string, unknown>;
  if (typeof record.model_slot !== 'string'
    || !(LOA_MODEL_SLOTS as readonly string[]).includes(record.model_slot)
...
export function parseLoaProfile(value: unknown): LoaProfile {
  ...
  const runtime = profile.runtime_requirements as Record<string, unknown>;
  ...
  for (const [key, path] of Object.entries(profile.paths as Record<string, unknown>)) {
  ...
  const modelSlots = Object.fromEntries(LOA_MODEL_SLOTS.map((slot) => {
    ...
    return [slot, record];
  })) as unknown as Record<LoaModelSlot, LoaProfile['model_slots'][LoaModelSlot]>;
  return {
    ...
    paths: profile.paths as LoaProfile['paths'],
...
export function loadLoaProfile(path: string): LoadedLoaProfile {
  ...
  try {
    ...
    throw new Error(`Loa profile is invalid JSON: ${
...
export function validateResolvedHost(
  value: unknown,
  profile: LoaProfile,
  ...
  const host = receipt.host as Record<string, unknown>;
  ...
  for (const role of LOA_ROLE_IDS) {
    ...
    for (const [profileField, hostField] of mechanicFields) {
      if (mapping[profileField] !== model[hostField]) {
        throw new Error(
          `role ${role} ${profileField}=${mapping[profileField]} does not match exact host slot ${mapping.model_slot}.${hostField}=${model[hostField]}`,
        );
      }
    }
  }
...
export function captureRuntimeSnapshot(options: CaptureRuntimeSnapshotOptions): RuntimeSnapshot {
  ...
  let runtimeBundle: VerifiedLoaBundle;
  try {
    for (const file of options.bundle.lock.files) {
      writeFileAtomic(
        join(runtimeBundleRoot, file.path),
        readLockedFile(options.bundle, file.path, file.classification),
        0o600,
      );
    }
    writeFileAtomic(
      join(runtimeBundleRoot, 'bundle.lock.json'),
      readVerifiedBundleLock(options.bundle),
      0o600,
    );
  ...
  const files = [
    ...runtimeBundle.lock.files.map((file) => ({
      ...
      digest: file.digest,
      byte_length: String(
        readLockedFile(runtimeBundle, file.path, file.classification).byteLength,
      ),
    })),
    {
  ...
  const runLocalProfilePath = join(
    runtimeBundleRoot,
    'adapters',
    'loa',
    'profiles',
  ...
  const base: Omit<RuntimeSnapshot, 'tree_digest'> = {
...
function verifyPinnedHostCapabilities(
  snapshotPath: string,
...
export function verifyRuntimeSnapshot(
  ...
  if (resolve(snapshot.profile.path) !== join(
    runtimeRoot,
    'bundle',
    'adapters',
    'loa',
  ...
  if (digest !== snapshot.tree_digest) throw new Error('runtime snapshot tree digest mismatch');
  return snapshot;
}
 ...
 */
export function loadPinnedHostCapabilities(
  snapshotPath: string,
  options: { allowSimulation?: boolean } = {},
): LoaHostCapabilities {
...
export function defaultProfilePath(bundleRoot: string): string {
