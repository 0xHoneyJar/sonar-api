...
import { sha256Digest, stableJsonBytes, writeJsonAtomic } from './fs.ts';
import { loadLoaProfile } from './runtime-snapshot.ts';
...
function parseCli(argv: string[]): ParsedCli {
  ...
  while (argv.length > 0) {
    ...
    if (option === '--profile') profilePath = argv.shift() || '';
    else if (option === '--output') outputPath = argv.shift() || '';
    else if (option === '--model') modelId = argv.shift() || '';
    else if (option === '--provider') provider = argv.shift() || '';
    else if (option === '--claude') claudePath = argv.shift() || '';
    else if (option === '--bwrap') sandboxPath = argv.shift() || '';
    else if (option === '--timeout-ms') timeoutMs = argv.shift() || '';
    else if (option === '--max-output-bytes') maxOutputBytes = argv.shift() || '';
    else if (option === '--max-budget-usd') maxBudgetUsd = argv.shift() || '';
    else if (option === '--json') json = true;
    else throw new Error(`unknown host attestation option: ${option || '<empty>'}`);
  }
...
export function runHostAttestationCli(argv = process.argv.slice(2)): number {
  try {
