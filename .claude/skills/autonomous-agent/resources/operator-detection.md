...
for sig in "${MOLTBOT_SIGNATURES[@]}"; do
  # OKF/ICM cycle: loa emits its OWN generated AGENTS.md (governance projection
  # via agents-md-gen.sh). That is NOT a Moltbot/operator signal — skip it.
  if [ "$sig" = "AGENTS.md" ] && [ -f "AGENTS.md" ] \
     && head -1 AGENTS.md 2>/dev/null | grep -q 'generated by agents-md-gen.sh'; then
    continue
  fi
  [ -f "$sig" ] && ((count++))
done
...
**Exception (OKF/ICM cycle):** a *loa-generated* `AGENTS.md` whose first line contains `generated by agents-md-gen.sh` is loa's own governance projection, **not** an operator marker — skip it. Only a hand-authored AGENTS.md with operator/agent configuration counts.
