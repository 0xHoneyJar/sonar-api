 ...
 */
export function detectRefs(text, currentRepo) {
    const refs = [];
    const seen = new Set();
    for (const pattern of GITHUB_REF_PATTERNS) {
        ...
        while ((match = regex.exec(text)) !== null) {
            ...
            if (currentRepo && repoSlug === currentRepo)
                continue;
 ...
 */
export function parseManualRefs(refs) {
    const result = [];
 ...
 */
export async function fetchCrossRepoContext(refs, logger, allowedOwners, exec = defaultExec) {
    const context = [];
    const errors = [];
    ...
    for (const ref of refs) {
        ...
        if (!ref.number)
            continue; // Can't fetch without a number
        ...
        if (ref.source === "auto") {
            ...
            if (!ownerAllowed) {
                const reason = `blocked: auto-detected owner '${ref.owner}' is not on the cross-repo allowlist`;
                logger?.warn(`[cross-repo] ${reason} - not fetching ${ref.owner}/${ref.repo}#${ref.number}`);
 ...
 */
async function fetchRepoVisibility(owner, repo, exec = defaultExec) {
    const { stdout } = await exec("gh", ["repo", "view", `${owner}/${repo}`, "--json", "visibility"], { timeout: PER_REF_TIMEOUT_MS });
 ...
 */
async function fetchRef(ref, exec = defaultExec, includeBody = true) {
...
//# sourceMappingURL=cross-repo.js.map