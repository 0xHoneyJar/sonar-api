...
#
# v1 SCOPE NOTE (deliberate, loud): every generated dispatch_contract points
# at the SHIPPED NO-OP example-*.sh phase scripts under
# .claude/skills/scheduled-cycle-template/contracts/ — the `phases:` config
# list is a LABEL only in v1 (all labels resolve to the same placeholder
# scripts). Real Flatline / Bridgebuilder / red-team dispatch is deliberately
# deferred to bd-fanout-real-dispatch-9jv6: bridgebuilder-review in particular
# is a Claude Code SKILL with no standalone CLI entrypoint and cannot be fired
# unattended from cron without new machinery. A post-reset run today produces
# an audit trail only — it does NOT re-run any real review.
#
...
# hand-edit. v1 PLACEHOLDER: dispatch_contract points at the shipped no-op
# example-*.sh phase scripts. Real "${phase}" dispatch is deferred to
# bd-fanout-real-dispatch-9jv6.
...
_plan() {
    ...
    for window in "${windows[@]}"; do
        local time_part tz_part hour minute final_minute
        time_part="${window%% *}"
        tz_part="${window#* }"
        hour="${time_part%%:*}"
        minute="${time_part##*:}"
        hour=$((10#$hour))
        minute=$((10#$minute))
        final_minute="$(_final_minute "$minute" "$offset")"
