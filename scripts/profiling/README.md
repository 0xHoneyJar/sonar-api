...
| `summarize-progress.mjs` | Computes p50/p95 rates, progress intervals, ETA, and hard invariants from sampler NDJSON |
...
Governance loop: update registry → `pnpm materialize:eth-floor` → `pnpm materialize:eth-floor:check` → `pnpm verify:historical-floors:enforce`. A KF-013 wipe/resume is a separate operator-authorized production action.
...
pnpm --silent summarize:mission-01 \
  .run/evidence/mission-01-progress.ndjson \
  --expected-start-block 12287507 \
  > .run/evidence/mission-01-summary.json
```
