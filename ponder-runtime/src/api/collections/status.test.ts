...
import { resolveCollectionStatus, toStatusResponse } from "./status";
import type { IngestJobRecord } from "./types";
...
describe("resolveCollectionStatus", () => {
  it("prefers indexed when holders exist", () => {
    expect(resolveCollectionStatus({ indexed: { holderCount: 10, indexedAtMs: 1 }, job: job("queued") }))
      .toBe("indexed");
  });

  it("maps queued jobs to indexing", () => {
    expect(resolveCollectionStatus({ indexed: { holderCount: 0, indexedAtMs: null }, job: job("queued") }))
      .toBe("indexing");
  });

  it("maps failed jobs to failed", () => {
    expect(resolveCollectionStatus({ indexed: { holderCount: 0, indexedAtMs: null }, job: job("failed") }))
      .toBe("failed");
  });
