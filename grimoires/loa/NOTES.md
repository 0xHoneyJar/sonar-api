...
- **SESSION 3 (2026-05-21) — Berachain ECOSYSTEM belt DEPLOYED + backfilled + validated (leg 1 of the 4-chain sovereign indexer).**
  ...
  - **DEPLOY (deliberate, not rushed — repoint deferred):** confirmed `--restart` is REQUIRED (envio `ChainManager.makeFromDbState` iterates DB chains, ignores new config chains on resume) → re-init re-syncs ALL 4. Steps when triggered: add Base/OP/ETH upstreams+networks to `erpc.yaml` (free, A+B safeguards + per-upstream getLogs verification per KF-012 — Base/OP are op-stack liars) → set `ENVIO_RESTART=1` → `railway up -s belt-indexer -c` → backfill (Bera ~60min eRPC + OP fast + Base/ETH measure, may need paid RPC) → reconcile → delete `ENVIO_RESTART` → redeploy. Keep `ENVIO_PG_SSL_MODE=false`. /backing down for the combined backfill.
