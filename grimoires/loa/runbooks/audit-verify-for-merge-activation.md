...
> **Tooling note:** the `audit-signing-helper.py trust-store-sign` subcommand is now implemented — it builds the exact signed payload `trust-store-verify` checks (JCS of `{schema_version, keys, revocations, trust_cutoff}`), signs with `--root-priv`, writes `root_signature`, and **refuses to sign** if the root key diverges from `--signer-pubkey-from` (the pinned pubkey) or if `schema_version` is missing; it self-verifies before emitting. The `audit-keys-bootstrap.md` Step 4 command works as written. **Run it ONLY on the air-gapped workstation that holds the root key** — never in a network-connected environment.
