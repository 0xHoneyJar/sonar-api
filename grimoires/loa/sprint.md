...
**MEDIUM — 7 tasks.** This sprint changes planning/runtime configuration and
...
- [x] Bounded all-attempt campaign receipt with no selected favorable run.
- [x] Allowlisted review environment and mechanical observational-admission
      classifier.
...
content, refusal, or truncation invalidate the transport cohort; an
unfavorable finding blocks the plan and requires adjudication. A later run
requires a new pre-dispatch declaration, remaining campaign budget, and an
all-attempt receipt that carries every earlier result. Transport-level success
...
Prepare and execute the separately authorized upstream Loa repair through
draft PR #1228, retaining issue #1199/#1231 linkage and review-gated merge.
...
qualify when raw content failed schema validation. A zero-finding response now
additionally requires a substantive `no_findings_reason` and non-empty
`reviewed_sections`; this guard was consumed at Sonar commit `54e7d47b` from
upstream PR #1228. Upstream stability remains owned by
`bd-v54z.4`/`bd-v54z.5`; Score consumption additionally depends on
`bd-v54z.6`, the complete, redacted, identity-bound receipt gate. Simstim must
carry all three dependencies plus readiness/MODELINV NON-READY status in its
input and output receipts.

### T0.5 Bounded review campaign

Aggregate every declared review into one campaign decision. A successful
transport or unanimous later attempt cannot supersede an earlier adverse
finding.
...
Run Flatline through `scripts/with-flatline-env.sh` and admit Simstim only
through `scripts/check-simstim-admission.mjs`.

**AC:** the environment wrapper passes only an explicit process/config-path/
certificate-path allowlist. The admission classifier fails closed unless the
receipt is `NON_AUTHORITATIVE`, readiness is `NON_READY`, provider-served
identity is false, Score emission and consumption readiness are false,
...
consumption blocked behind `bd-v54z.2`, `bd-v54z.4`, `bd-v54z.5`, and
`bd-v54z.6`; production invariants are sampled before and after review;
observational Simstim input and output must independently pass the admission
classifier.

### Dependencies

- `0xHoneyJar/loa#1228` — content-qualified quorum and reasoned-empty repair;
  consumed from full upstream head
  `bfb3d81cbe4121a9f652793b861a5af95bbfc5e3` at Sonar commits `13403ee3`
  and `54e7d47b`. The unmerged pin is owned by `bd-v54z.4`, expires at
  `2026-07-22T02:00:00Z`, and is mechanically refused after expiry. Merge or
  replace it before Score consumption.
- `0xHoneyJar/loa#1231` / `bd-v54z.5` — reasoned no-findings contract is
  implemented in PR #1228 but stays open until upstream checks and merge/pin
  proof complete.
...
| Risk | Mitigation |
|---|---|
| CLI alias differs from the model actually served | Freeze version/hash/alias evidence, classify it as observational, and block Score on `bd-v54z.6`; fail on authoritative returned-ID mismatch. “Authoritative” means a machine-readable model ID returned by the serving runtime or an operator-attested immutable alias map, never the requested CLI argument alone. |
...
- Review subprocesses run through `scripts/with-flatline-env.sh`, which builds
  a new environment from an explicit allowlist of process, config/cache-path,
  locale, certificate-path, and output-control variables. Provider keys,
  database URLs, Railway tokens, and arbitrary secret variables are not
  inherited; values are neither inspected nor recorded.
...
- `bats tests/unit/flatline-env-allowlist.bats` exits 0.
- `bats tests/unit/simstim-admission.bats` exits 0.
- The pre-dispatch and post-dispatch Simstim receipts independently pass
  `node scripts/check-simstim-admission.mjs <receipt>`.
...
- The v1.198.7 Flatline readiness result remains non-ready (`NO_API_KEYS`,
  ...
  ready.
- No unmanaged `.claude/` changes are introduced; both framework repair deltas
  use the governed update lane and refresh strict integrity hashes.
...
7. `54e7d47b` — reasoned-empty quorum guard consumption;
8. `7ffec69f` — allowlisted review environment;
9. `ffd33721` + `8b6fa62c` — observational admission and pin-expiry gates.
...
- Any review beyond the current campaign's fixed attempt budget.
- Any admission receipt that omits a prior campaign finding, graduation
  dependency, complete artifact manifest, or live upstream pin.
- Any readiness promotion, Score-facing artifact, or identity-verified claim
  rejected by `scripts/check-simstim-admission.mjs`.
