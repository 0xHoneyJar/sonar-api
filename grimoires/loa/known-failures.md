...
**Status**: RESOLVED-VIA-WORKAROUND 2026-05-21 — **the `--restart`-seeds-then-resume pattern** (NOT `ENVIO_PG_SSL_MODE=false` — that was a 2026-05-20 MISDIAGNOSIS, corrected below). The Rust-CLI persisted_state upsert fails 28P01 on a TRUE fresh init (`envio start --restart`) regardless of sslmode; the **resume path skips the fatal upsert**. Workaround: let `--restart` wipe + (its JS init) seed the schema + all `chain_metadata` rows — it crashes on the Rust upsert AFTER seeding — then **delete `ENVIO_RESTART` + redeploy → resume**, which backfills the seeded chains from their start_blocks (chain_metadata `progressBlockNumber=-1` → start_block). The belt runs fine without `persisted_state` (resume reads `chain_metadata`/`checkpoints`, never `persisted_state` — verified in `resumeInitialState`). Root cause of the Rust-CLI 28P01 is unsolved but does not matter (resume sidesteps it). Config-only; sovereign path preserved.
...
| 2026-05-21 | `ENVIO_PG_SSL_MODE=false` (valid `Bool(false)` → SSL off) — believed to fix the Rust-CLI auth | **MISLEADING PARTIAL** — 28P01 appeared "gone" but ONLY because that deployment had `ENVIO_RESTART` deleted → it hit the **resume** path (which skips the upsert), not because sslmode=false fixed the upsert. The "next error was start_block" was the resume reaching contract-validation. Misattributed at the time. | rebuild 16387281 (resume, not fresh-init) |
| 2026-05-21 | **4-chain re-init via `--restart` with `ENVIO_PG_SSL_MODE=false` already set** | **REPRODUCED 28P01 on fresh-init** — proves sslmode=false does NOT fix it; the crash is specific to the Rust-CLI persisted_state upsert on `--restart`. The `--restart` JS init DID seed schema + all 4 `chain_metadata` rows (1/10/8453/80094 @ -1) before the Rust crash. Railway marked the deployment CRASHED after the loop. | deployment f6bd4eab CRASHED; chain_metadata 4 rows seeded |
| 2026-05-21 | **Delete `ENVIO_RESTART` + redeploy → resume** (on the seeded schema) | **RESOLVED** — resume skips the fatal upsert; all 4 chains backfill from their seeded start_blocks (Bera 8221 / Base 2430439 / OP 107558369 / ETH 13090020). This is the canonical re-init pattern: `--restart` to seed, then resume to backfill. | deployment 1e2a9988; 4 chains advancing (ETH ~35k blk/s, Bera ~2.5k, OP/Base started) |

### Reading guide

If an envio indexer **crash-loops on `Failed to upsert persisted state table → password authentication failed`** on a `--restart` (fresh init), while you can independently confirm the password is correct (connect with the same creds) AND the schema/`chain_metadata` tables clearly got created (the JS layer authed fine): do **not** chase the password, and do **not** expect `ENVIO_PG_SSL_MODE=false` to fix it (it doesn't — that was the original misdiagnosis; the Rust-CLI upsert still 28P01s with sslmode=false). **The fix is the `--restart`-seeds-then-resume pattern:**
